...
<%			if nounsubscribe<>TRUE then %>
			<div class="ectdivcontainer">
				<div class="ectdivleft"><input type="checkbox" class="ectcheckbox" name="allowemail" value="ON"<% if allowemail<>0 then print " checked=""checked"""%> /></div>
...
<%			if nounsubscribe<>TRUE then %>
					<div class="ectdivcontainer">
						...
						<div class="ectdivright"><% if noconfirmationemail<>TRUE AND allowemail<>0 AND isconfirmed=0 then print xxWaiCon else print "<input type=""checkbox"" class=""ectcheckbox"" name=""allowemail"" value=""ON""" & IIfVr(allowemail<>0, " checked=""checked""", "") & " disabled=""disabled"" />"%></div>
					</div>
<%			end if %>
					<div class="ectdivcontainer">
						<div class="ectdivleft"><%=xxEmail%></div>
...
<%				sSQL="SELECT listID,listName,listAccess FROM customerlists WHERE listOwner=" & SESSION("clientID") & " ORDER BY listName"
				...
				if NOT rs.EOF then
					do while NOT rs.EOF
						...
						print "<div class=""ectdiv2column""><div class=""ectpublicaccesstext"">" & xxPubAcc & ":</div><div class=""ectpublicaccessinput""><input class=""ecttextinput giftregistrycopy"" id=""publicop" & rs("listID") & """ value=""" & storeurl & "cart" & extension & "?pli=" & rs("listID") & "&pla=" & rs("listAccess") & """ size=""60"" /></div></div>"
...
%>					<div>
						...
						<input type="text" class="ecttextinput createlistinput" name="listname" size="40" maxlength="50" placeholder="<%=xxLisNam%>" />
					</div>
				  </div>
			  </div>
<%			end if
