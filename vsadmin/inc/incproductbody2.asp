...
if IsEmpty(showcategories) OR showcategories=TRUE then
	if NOT usecsslayout then print "<tr><td colspan=""" & productcolumns & """>"
	...
	if NOT usecsslayout then print "</td></tr>" & vbCrLf
end if
...
if globaldiscounttext<>"" then
	...
	if rs.EOF then
		...
		do while NOT rs.EOF AND localcount<rs.PageSize
			...
			print "<div class=""" & cs & "product " & trim(rs("pCustomCSS") & " " & rs("pId")) & """>" & vbCrLf
			...
			if isarray(prodoptions) then
				if NOT noproductoptions OR instr(quickbuylayout,"options")>0 then
					...
					optionshtml=displayproductoptions("<span class=""prodoption"">","</span>", optdiff, thetax,FALSE,hasmultipurchase,optjs)
			...
			if (localcount MOD productcolumns=0) AND NOT usecsslayout then
				...
				if noproductseparator<>TRUE then
					if Not rs.EOF And localcount < adminProdsPerPage then
						print "<tr>"
						for index=1 to productcolumns
							print "<td class=""prodseparator"">" & IIfVr(prodseparator<>"", prodseparator, "<hr class=""prodseparator"" width=""70%"" align=""center""/>") & "</td>"
						next
						print "</tr>"
					end if
				end if
