	...
	if (sidefilter AND 4)=4 then ' Price bands
		...
		<div style="display:table;margin-top:4px;width:100%">
			<div style="width:50%;text-align:center;display:table-cell">
				<div>
					<input class="ecttextinput sfprice" id="paminprice" type="text" onchange="spriceset=true" value="<%=paminprice%>" placeholder="Min Price" />
				</div>
				<div>
			...
			<div style="width:50%;text-align:center;display:table-cell">
				<div>
					...
					<input class="ecttextinput sfprice" id="pamaxprice" type="text" onchange="spriceset=true" value="<%=pamaxprice%>" placeholder="Max Price" />
				</div>
				<div>
...
<%		print "</div>"
	...
	if (sidefilter AND 8)=8 then ' Sort Order
		...
		print "<div>"
		%><select class="ectselectinput sidefilter" size="1" onchange="changesflocation('sortby',this)">
...
%>		</select><%
	...
	if (sidefilter AND 16)=16 then
		...
		print "<div>"
		%><select class="ectselectinput sidefilter" size="1" onchange="changesflocation('perpage',this)">
<%		for index=1 to 5
...
%>			  </select><%
	...
	if (sidefilter AND 32)=32 then
		...
		print "<div style=""display:table;width:100%""><div style=""display:table-row""><div style=""display:table-cell""><input onkeydown=""changesffiltertext(event.keyCode,this)"" type=""text"" class=""ecttextinput sidefilter"" style=""width:100%;box-sizing:border-box"" id=""paectfilter"" name=""pafilter"" value="""&htmlspecials(getget("filter"))&""" /></div><div style=""display:table-cell;padding-left:5px"">" & imageorbutton(imgfilterproducts,"&raquo;","sidefilter sidefiltergo","filtergo()",TRUE) & "</div></div></div>"
		print "</div>"
	end if
	end select
next
