...
wantmanufacturer=instr(productpagelayout&quickbuylayout,"manufacturer")>0 OR googletagid<>""
...
if getpost("mode") <> "checkout" AND getpost("mode") <> "add" AND getpost("mode") <> "go" AND getpost("mode") <> "paypalexpress1" AND getpost("mode") <> "authorize" then
	...
	for csindex=0 to UBOUND(crosssellactionarr)
		...
		if relatedlist<>"" then
			...
			sSQL="SELECT "&IIfVs(mysqlserver<>TRUE AND crosssellsectionmax<>"","TOP "&crosssellsectionmax&" ")&"pId,pSKU,"&getlangid("pName",1)&","&WSP&"pPrice,pListPrice,pSection,pSell,pStockByOpts,pStaticPage,pStaticURL,pInStock,pExemptions,pTax,pTotRating,pNumRatings,pBackOrder,pCustomCSS,pCustom1,pCustom2,pCustom3,pDateAdded,pMinQuant,pSchemaType,"&IIfVs(wantmanufacturer,getlangid("scName",131072)&",")&IIfVs(NOT noblankdescription,"'' AS ")&getlangid("pDescription",2)&","&getlangid("pLongDescription",4)&" FROM products "&IIfVs(wantmanufacturer,"LEFT OUTER JOIN searchcriteria on products.pManufacturer=searchcriteria.scID ")&"WHERE " & IIfVs(ectsiteid<>"", "pSiteID=" & ectsiteid & " AND ") & "pDisplay<>0 AND pId IN (" & relatedlist & ")"
