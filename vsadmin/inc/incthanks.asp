	...
	if success then
		if ordAuthNumber="" then
			sXML=ppsoapheader(username, data2pwd, data2hash) & _
				...
				"    <NotifyURL>" & storeurlssl & "vsadmin/ppconfirm.asp</NotifyURL>" & _
				"        </PaymentDetails>" & _
...
if success AND googletagid<>"" AND is_numeric(ordID) then
	if ordID<>0 then
		sSQL="SELECT cartID,cartProdID,cartProdName,cartProdPrice,cartQuantity,sectionName,pSKU,pManufacturer FROM cart INNER JOIN products ON cart.cartProdID=products.pID INNER JOIN sections ON products.pSection=sections.sectionID WHERE cartOrderID='" & escape_string(ordID) & "' ORDER BY cartID"
		rs.open sSQL,cnn,0,1
		totalgoods=0
		ao=""
		index=1
		do while NOT rs.EOF
			...
			rs2.open sSQL,cnn,0,1
			if NOT rs2.EOF then coPriceDiff=rs2("coPrDff") else coPriceDiff=0
			rs2.close
			if NOT isnull(coPriceDiff) then totalgoods=totalgoods + (rs("cartProdPrice")+coPriceDiff)*rs("cartQuantity")
			sSQL="SELECT scName FROM searchcriteria WHERE scID=" & rs("pManufacturer")
			rs2.open sSQL,cnn,0,1
			if NOT rs2.EOF then brand=rs2("scName") else brand=""
			rs2.close
			if ao<>"" then ao=ao & ","
			ao=ao & "{item_id:'" & jsescapel(rs("cartProdID")) & "'," & _
				"item_name:'" & jsescapel(rs("cartProdName")) & "'," & _
				"index:" & index & ","
			if brand<>"" then ao=ao & "item_brand:'" & jsescapel(brand) & "',"
				ao=ao & "item_category:'" & jsescapel(rs("sectionName")) & "',"
				ao=ao & "price:" & jsescapel(rs("cartProdPrice")) & "}"
			index=index+1
			rs.MoveNext
		loop
		rs.close
		print "<script>gtag(""event"",""purchase"",{ currency:'" & countryCurrency & "',value:" & totalgoods & ",shipping:" & ordShipping & ",tax:" & (ordStateTax+ordHSTTax+ordCountryTax) & ",items:[" & ao & "]});</script>" & vbLf
	end if
elseif success AND (googleanalyticsinfo=TRUE OR usegoogleuniversal) AND is_numeric(ordID) then
	if ordID<>0 then
		session.LCID=1033
