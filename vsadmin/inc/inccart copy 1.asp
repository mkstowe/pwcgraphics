...
Dim iTotItems,international,multipleoptions,shipMethod,shipArr,shipcountry,intShipping(7,40),outofstockarr(),havematch,dHighest(10),dHighWeight,dTotalWeight,dTotalWeightOz,thePQuantity,thepname,thepprice,thepweight,xmlfnheaders
...
fromshipselector=FALSE : nodiscounts=FALSE : usehst=FALSE : multipleoptions=FALSE : stockwarning=FALSE : backorder=FALSE : cartEmpty=FALSE : handlingeligableitem=FALSE : noshowcart=FALSE : isavsmismatch=FALSE
...
alldata="" : shipMethod="" : WSP="" : OWSP="" : appliedcouponname="" : ordAVS="" : ordCVV="" : stateAbbrev="" : international="" : cpnmessage="" : cpnerror="" : shipselectoraction="" : altrate="" : backorderlist=""
appliedcouponamount=0 : totalquantity=0 : statetaxfree=0 : countrytaxfree=0 : shipfreegoods=0 : : shipdiscountexempt=0 : numshipdiscountexempt=0 : totalgoods=0 : handlingeligablegoods=0 : shippingtax=0
freeshippingincludeshandling=FALSE : somethingToShip=FALSE : freeshippingapplied=FALSE : warncheckspamfolder=FALSE : homecountry=FALSE : gotcpncode=FALSE : freeshipmethodexists=FALSE
errordname=FALSE : errordaddress=FALSE : errordcity=FALSE : errordstate=FALSE : errordshipstate=FALSE : errordcountry=FALSE : errordzip=FALSE : errordphone=FALSE : errordemail=FALSE : errtermsandconditions=FALSE : errordshipaddress=FALSE : errordshipcountry=FALSE
...
ordShipName="":ordShipLastName="":ordShipAddress="":ordShipAddress2="":ordShipCity="":ordShipState="":ordShipZip="":ordShipCountry="":ordShipPhone=""
if imgcheckoutbutton="" then imgcheckoutbutton="images/checkout.gif"
if imgcheckoutbutton2="" then imgcheckoutbutton2=imgcheckoutbutton
if imgcheckoutbutton3="" then imgcheckoutbutton3=imgcheckoutbutton
...
if instr(1,thefrompage,"javascript:",1)>0 OR instr(1,thefrompage,"cart.asp",1)>0 OR instr(1,thefrompage,"thanks.asp",1)>0 then thefrompage=""
		...
		if(smen=document.getElementById(stateid+'state')){
			...
			if(countryhasstates[cntid]){
				...
				for(var cind=0;cind<dynst[cntid].length;cind++){
					...
					smen.options[cind+1].innerHTML=((cntid==1||cntid==2)&&<%= IIfVr(mobilebrowser,"true","false") %>?dynab[cntid][cind]:dynst[cntid][cind]);
				}
				smen.length=cind+1;
...
sistateabb="" : sisstateabb=""
if sistate<>"" then
	sSQL2="SELECT stateName,stateAbbrev FROM states WHERE statename='" & escape_string(sistate) & "' OR stateAbbrev='" & escape_string(sistate) & "'"
	rs.open sSQL2,cnn,0,1
	if NOT rs.EOF then
		sistate=rs("stateName")
		sistateabb=rs("stateAbbrev")
	end if
	rs.close
end if
if sisstate<>"" then
	sSQL2="SELECT stateName,stateAbbrev FROM states WHERE statename='" & escape_string(sisstate) & "' OR stateAbbrev='" & escape_string(sisstate) & "'"
	rs.open sSQL2,cnn,0,1
	if NOT rs.EOF then
		sisstate=rs("stateName")
		sisstateabb=rs("stateAbbrev")
	end if
	rs.close
end if
%>		var initstate=[['<%=jsescape(sistate)%>','<%=jsescape(sisstate)%>'],['<%=jsescape(sistateabb)%>','<%=jsescape(sisstateabb)%>']];
		var gotstate=false;
		if(document.getElementById(isshp+"state")){
			var smen=document.getElementById(isshp+"state");
			for(var cind=0; cind<smen.length; cind++){
				if(smen.options[cind].value==initstate[0][isshp=='s'?1:0]||smen.options[cind].value==initstate[1][isshp=='s'?1:0]){
...
<%	currcountry=0
	...
	do while NOT rs.EOF
		if currcountry<>rs("stateCountryID") then
			...
			print "dynst[" & currcountry & "]=new Array();dynab[" & currcountry & "]=new Array();countryhasstates[" & currcountry & "]=" & currcountry & ";" & vbCrLf
		end if
		print "adst(" & currcountry & ",'" & jsescape(rs(getlangid("stateName",1048576))) & "','" & IIfVr(usestateabbrev=TRUE AND (currcountry=1 OR currcountry=2),jsescape(rs("stateAbbrev")),IIfVr(rs(getlangid("stateName",1048576))<>rs("stateName"),rs(getlangid("stateName",1048576)),"")) & "');" & vbCrLf
		rs.movenext
	loop
	rs.close
end sub
...
function getshiplogo(stype)
	if stype=3 then
		getshiplogo="<img src=""images/usps_logo.gif"" alt=""USPS Logo"" "&IIfVs(mobilebrowser,"width=""40px"" ")&"/>"
	elseif stype=4 then
		getshiplogo="<img src=""images/upslogo.png"" alt=""UPS Logo"" "&IIfVs(mobilebrowser,"width=""40px"" ")&"/>"
	elseif stype=6 then
		getshiplogo="<img src=""images/canadapost.gif"" alt=""CanadaPost Logo"" "&IIfVs(mobilebrowser,"width=""40px"" ")&"/>"
	elseif stype=7 OR stype=8 then
		getshiplogo="<img src=""images/fedexlogo.png"" alt=""FedEx Logo"" "&IIfVs(mobilebrowser,"width=""40px"" ")&"/>"
	elseif stype=9 then
		getshiplogo="<img src=""images/dhllogo.gif"" alt=""DHL Logo"" "&IIfVs(mobilebrowser,"width=""40px"" ")&"/>"
	else
		getshiplogo="<img src="""&IIfVr(shippinglogo<>"",shippinglogo,"images/defaultshiplogo.png")&""" alt=""Logo"" "&IIfVs(mobilebrowser,"width=""40px"" ")&"/>"
	end if
end function
sub writealtshipline(altmethod,altid,pretext,defpretext)
	if shippingoptionsasradios=TRUE then
		if altmethod<>"" OR origShipType=altid then print "<div class=""shipline""" & IIfVs(shipType=altid," style=""font-weight:bold""") & "><label class=""shipradio""><input type=""radio"" class=""shipradio"" value="""&altid&""""&IIfVr(shipType=altid," checked=""checked""","")&" onclick=""selaltrate("&altid&")"" /><span class=""shiplinetext"">" & IIfVr(shipType=altid,defpretext,pretext) & altmethod & "</span></label></div>"
	else
...
if SESSION("couponapply")<>"" then
	ect_query("UPDATE coupons SET cpnNumAvail=cpnNumAvail+1 WHERE cpnID IN (0" & SESSION("couponapply")&")")
	SESSION("couponapply")=""
end if
...
function checkuserblock(thepayprov)
	...
	if thepayprov<>"7" AND thepayprov<>"13" AND len(REMOTE_ADDR)<=15 then
		...
		if blockmultipurchase<>"" AND shipselectoraction="" then
	...
	if NOT nodiscounts then
		...
		if IsArray(cdalldata) then
			for index=0 to UBOUND(cdalldata,2)
				...
				loop
				rs.close
				sSQL="SELECT DISTINCT cpnID,cpnDiscount,cpnType,cpnNumber,cpnName,cpnThreshold,cpnQuantity,cpnThresholdRepeat,cpnQuantityRepeat FROM coupons LEFT OUTER JOIN cpnassign ON coupons.cpnID=cpnassign.cpaCpnID WHERE cpnNumAvail>0 AND cpnEndDate>=" & vsusdate(DateAdd("h",dateadjust,Now()))&" AND (cpnIsCoupon=0"
		...
		sSQL="SELECT DISTINCT cpnID,cpnDiscount,cpnType,cpnNumber,cpnName,cpnSitewide,cpnThreshold,cpnThresholdMax,cpnQuantity,cpnQuantityMax,cpnThresholdRepeat,cpnQuantityRepeat FROM coupons WHERE cpnNumAvail>0 AND cpnEndDate>=" & vsusdate(DateAdd("h",dateadjust,Now()))&" AND (cpnIsCoupon=0"
		if cdcpncode<>"" then sSQL=sSQL & " OR (cpnIsCoupon=1 AND cpnNumber='"&cdcpncode&"')"
...
sub calculateshippingdiscounts(subcpns)
	...
	if NOT nodiscounts then
		sSQL="SELECT cpnID,cpnName,cpnNumber,cpnDiscount,cpnThreshold,cpnCntry,cpnHandling FROM coupons WHERE cpnType=0 AND cpnSitewide=1 AND cpnNumAvail>0 AND cpnThreshold<="&(totalgoods-(shipdiscountexempt+IIfVr(shippingafterproductdiscounts,totaldiscounts,0)))&" AND (cpnThresholdMax>"&(totalgoods-(shipdiscountexempt+IIfVr(shippingafterproductdiscounts,totaldiscounts,0)))&" OR cpnThresholdMax=0) AND cpnQuantity<="&(totalquantity-numshipdiscountexempt)&" AND (cpnQuantityMax>"&(totalquantity-numshipdiscountexempt)&" OR cpnQuantityMax=0) AND cpnEndDate>=" & vsusdate(DateAdd("h",dateadjust,Now()))&" AND (cpnIsCoupon=0 OR (cpnIsCoupon=1 AND cpnNumber='"&rgcpncode&"'))" & _
			" AND ((cpnLoginLevel>=0 AND cpnLoginLevel<="&minloglevel&") OR (cpnLoginLevel<0 AND -1-cpnLoginLevel="&minloglevel&"))"
		rs.open sSQL,cnn,0,1
		do while NOT rs.EOF
		...
		if shipType>=1 then
			...
			for index=0 to UBOUND(intShipping,2)
				if intShipping(3,index)=TRUE then
					...
					if intShipping(4,index) AND freeshippingapplied then freeshipamnt=intShipping(2,index) - intShipping(7,index)
				end if
			next
		end if
...
function initshippingmethods()
	...
	elseif shipType=3 OR shipType=4 OR shipType>=6 then ' USPS / UPS / Canada Post / Fedex / SmartPost / DHL
		if shipType=4 OR shipType=7 OR shipType=8 then shipinsuranceamt=""
		if shipType=3 then
		...
		elseif shipType>=8 then
			sSQL=",uspsLocal FROM uspsmethods WHERE uspsID>"&(shipType-4)&"00 AND uspsID<"&(shipType-3)&"00"
		end if
		...
		if packtogether then
			...
			if islastpacktogether(apsrs,prodindex) AND thepweight>0 then
				...
				if international<>"" then
					...
					sXML=sXML & addUSPSDomestic(rowcounter,"Parcel",origZip,destZip,thepweight / numpacks,numpacks,"REGULAR","True")
				end if
			...
			if shipThisProd then
				...
				if international<>"" then
					...
					sXML=sXML & addUSPSDomestic(rowcounter,"Parcel",origZip,destZip,thepweight / numpacks,apsrs(4,prodindex)*numpacks,"REGULAR","True")
				end if
				rowcounter=rowcounter + 1
				zeropackdims()
			end if
...
function calculateshipping()
	...
	elseif shipType=9 AND somethingToShip then
		...
		sXML=sXML & "</Pieces><PaymentAccountNumber>" & DHLAccountNo & "</PaymentAccountNumber><IsDutiable>" & IIfVr(origCountryCode=shipCountryCode OR (iseuropean(origCountryCode) AND iseuropean(shipCountryCode)),"N","Y") & "</IsDutiable>" & _
			"<NetworkTypeCode>AL</NetworkTypeCode></BkgDetails><To><CountryCode>" & shipCountryCode & "</CountryCode><Postalcode>" & destZip & "</Postalcode>" & IIfVr(zipisoptional(shipCountryID) OR shipCountryID=65,"<City>"&ordCity&"</City>","") & "</To>" & _
...
sub writeshippingoption(shipcost,freeshipexempt,freeship,shipmethod,isselected,wsodelivery)
	if freeshippingapplied AND freeship=1 then wsofreeshipamnt=(shipcost-freeshipexempt) else wsofreeshipamnt=0
	...
	if freeship=1 AND freeshippingincludeshandling=TRUE then wsohandling=0
	if shippingoptionsasradios=TRUE then
		print "<div class=""shiprateline""><label class=""shipradio""><input type=""radio"" class=""shipradio"" value=""RATE"""&IIfVr(isselected, " checked=""checked""", "")&" onclick=""updateshiprate(this,"&numshiprate&")"" /><span class=""shipratemethod"&IIfVs(numshiprateingroup=0," shipratemethodselected")&""">"&shipmethod&" "&IIfVr(combineshippinghandling, FormatEuroCurrency((shipcost+wsohandling)-wsofreeshipamnt), FormatEuroCurrency(shipcost-wsofreeshipamnt))&"</span>"&IIfVs(wsodelivery<>"","<div class=""cart3servicecommitment"">(" & wsodelivery & ")</div>")&"</label></div>"
	else
...
function showshippingselect()
	...
	if shipType>=1 then
		if shippingoptionsasradios<>TRUE then
			print "<select size=""1"" onchange=""updateshiprate(this,(this.selectedIndex"&IIfVr(fromshipselector,"-1","")&")+"&numshiprate&")"">"
		...
		for index=0 to UBOUND(intShipping,2)
			if intShipping(3,index) then
				...
				call writeshippingoption(vsround(intShipping(2,index), 2), vsround(intShipping(7,index), 2), intShipping(4,index), intShipping(0,index), index=selectedshiptype, intShipping(1,index))
			end if
		next
		if shippingoptionsasradios<>TRUE then print "</select>"
...
sub insuranceandtaxaddedtoshipping()
	if (is_numeric(shipinsuranceamt) OR useuspsinsurancerates=TRUE) AND somethingToShip then
		if ((wantinsurance_ AND abs(addshippinginsurance)=2) OR abs(addshippinginsurance)=1) AND useuspsinsurancerates=TRUE AND shipType=3 then
			for index3=0 to UBOUND(intShipping,2)
				intShipping(2,index3)=intShipping(2,index3) + getuspsinsurancerate(cdbl(totalgoods))
			next
			shipping=shipping + getuspsinsurancerate(cdbl(totalgoods))
		elseif NOT is_numeric(shipinsuranceamt) then
			' Nothing
		elseif (wantinsurance_ AND addshippinginsurance=2) OR addshippinginsurance=1 then
			for index3=0 to UBOUND(intShipping,2)
...
sub checkdeletecart(thecartid)
	sSQL="SELECT cartID,cartListID,cartClientID,listOwner,cartProdID FROM cart LEFT JOIN customerlists ON cart.cartListID=customerlists.listID WHERE (cartCompleted=0 OR cartCompleted=3) AND cartID=" & IIfVr(is_numeric(thecartid), thecartid, 0) & " AND " & getsessionsql()
	rs2.open sSQL,cnn,0,1
	if NOT rs2.EOF then
		...
		if rs2("cartListID")>0 AND listowner<>SESSION("clientID") then
			...
			if rs2("cartProdID")=giftwrappingid then ect_query("UPDATE cart SET cartGiftWrap=0 WHERE " & getsessionsql())
...
sub writeshippingflags(costage)
	hasshipflag=FALSE
	if willpickup_ OR commercialloc OR commercialloc=2 OR saturdaydelivery OR abs(addshippinginsurance)=2 OR (allowsignaturerelease AND signatureoption<>"") OR insidedelivery OR holdatlocation OR homedelivery then hasshipflag=TRUE
	if hasshipflag then print "<div class=""coshipflagscontainer"">"
	if willpickuptext<>"" OR willpickup_ then %>
			<div class="billformrow"><div class="cobhl cobhl<%=costage%> cdshipftflag cdformtwillpickup"><input type="checkbox" name="willpickup" value="Y" <%
				if willpickup_ then print "checked=""checked"" "
				if costage=3 then print "onchange=""setchangeflag(this.checked,'w')"" "%>/></div>
			<div class="cobll cobll<%=costage%> cdshipflag cdformwillpickup"><%=willpickuptext & IIfVr(willpickupcost<>""," (" & FormatEuroCurrency(willpickupcost) & ")","")%></div></div>
<%	end if
	if NOT willpickup_ then
		if commercialloc=TRUE OR commercialloc=2 then %>
			<div class="billformrow"><div class="cobhl cobhl<%=costage%> cdshipftflag"><input type="checkbox" name="commercialloc" value="Y" <%
			...
			<div class="cobll cobll<%=costage%> cdshipflag"><%=xxComLoc%></div></div>
<%		end if
		if saturdaydelivery=TRUE then %>
			<div class="billformrow"><div class="cobhl cobhl<%=costage%> cdshipftflag"><input type="checkbox" name="saturdaydelivery" value="Y" <%
				if (ordComLoc AND 4)=4 then print "checked=""checked"" "
				if costage=3 then print "onchange=""setchangeflag(this.checked,2)"" "%>/></div>
			<div class="cobll cobll<%=costage%> cdshipflag"><%=xxSatDel%></div></div>
<%		end if
		if abs(addshippinginsurance)=2 then %>
			<div class="billformrow"><div class="cobhl cobhl<%=costage%> cdshipftflag"><%
				if forceinsuranceselection AND costage<>3 then
					print "<select name=""wantinsurance"" size=""1""><option value="""">"&xxPlsSel&"</option><option value="""">"&xxNo&"</option><option value=""Y"">"&xxYes&"</option></select>"
				else
					print "<input type=""checkbox"" name=""wantinsurance"" value=""Y"" "&IIfVs((ordComLoc AND 2)=2,"checked=""checked"" ")&IIfVs(costage=3,"onchange=""setchangeflag(this.checked,1)"" ")&"/>"
				end if %></div>
			<div class="cobll cobll<%=costage%> cdshipflag"><%=IIfVr(forceinsuranceselection,xxChoIns,xxWantIns)%></div></div>
<%		end if
		if allowsignaturerelease=TRUE AND signatureoption<>"" then %>
			<div class="billformrow"><div class="cobhl cobhl<%=costage%> cdshipftflag"><input type="checkbox" name="signaturerelease" value="Y" <%
				if (ordComLoc AND 8)=8 then print "checked=""checked"" "
				if costage=3 then print "onchange=""setchangeflag(this.checked,3)"" "%>/></div>
			<div class="cobll cobll<%=costage%> cdshipflag"><%=xxSigRel%></div></div>
<%		end if
		if insidedelivery=TRUE then %>
			<div class="billformrow"><div class="cobhl cobhl<%=costage%> cdshipftflag"><input type="checkbox" name="insidedelivery" value="Y" <%
				if (ordComLoc AND 16)=16 then print "checked=""checked"" "
				if costage=3 then print "onchange=""setchangeflag(this.checked,4)"" "%>/></div>
			<div class="cobll cobll<%=costage%> cdshipflag"><%=xxInsDel%></div></div>
<%		end if
		if holdatlocation=TRUE then %>
			<div class="billformrow"><div class="cobhl cobhl<%=costage%> cdshipftflag"><input type="checkbox" name="holdatlocation" value="Y" /></div>
			<div class="cobll cobll<%=costage%> cdshipflag">Please click here to Hold at Location</div></div>
<%		end if
		if homedelivery=TRUE then %>
			<div class="billformrow"><div class="cobhl cobhl<%=costage%> cdshipftflag">Delivery Options</div>
			<div class="cobll cobll<%=costage%> cdshipflag"><select name="homedelivery" size="1">
...
<%		end if
	...
	if termsandconditions AND costage=3 AND (ordPayProvider="19" OR ordPayProvider="21") then %>
		<div class="billformrow">
		  <div class="cobhl cobhl2 cdshiptterms"><input type="checkbox" name="termsandconds" id="termsandconds" value="1" onclick="document.getElementById('sftermsandconds').value=this.checked?1:''" <%=IIfVs(getpost("sftermsandconds")="1","checked=""checked"" ")%>/></div>
		  <div class="cobll cobll2 cdshipterms"><%=xxTermsCo%></div>
		</div>
<%
	end if
...
elseif left(getget("token"),2)="EC" AND checkoutmode<>"paypalcancel" then ' { PayPal Express
	...
	if callxmlfunction("https://" & IIfVs(demomode,"SHA2-test-") & "api" & IIfVr(data2hash<>"", "-3t", "") & sandbox & ".paypal.com/2.0/", sXML, res, IIfVr(data2hash<>"","",username), "WinHTTP.WinHTTPRequest.5.1", errormsg, FALSE) then
		countryid=0
		success=FALSE
		ordPayProvider="19"
		ordEmail=""
		...
		if NOT gotaddress then
			response.redirect storeurl & "cart.asp"
			cartisincluded=TRUE
		elseif success then
			paypalexpress=TRUE
...
elseif checkoutmode="paypalexpress1" OR checkoutmode="billmelater" then ' }{ PayPal Express
	...
	sXML=ppsoapheader(username, data2pwd, data2hash) & _
		...
		"<ReturnURL>" & storeurlssl & "cart.asp</ReturnURL><CancelURL>" & storeurlssl & "cart.asp?action=paypalcancel</CancelURL>" & _
	...
	elseif callxmlfunction("https://" & IIfVs(demomode,"SHA2-test-") & "api" & IIfVr(data2hash<>"", "-3t", "") & sandbox & ".paypal.com/2.0/", sXML, res, IIfVr(data2hash<>"","",username), "WinHTTP.WinHTTPRequest.5.1", errormsg, FALSE) then
		set xmlDoc=Server.CreateObject("MSXML2.DOMDocument")
		xmlDoc.validateOnParse=FALSE
		xmlDoc.loadXML (res)
		Set nodeList=xmlDoc.getElementsByTagName("SOAP-ENV:Body")
...
function additemtocart(ainame,aiprice)
	...
	rs.open sSQL,cnn,0.1
	cartfloodcontrol=cint(rs("cartcnt"))>1000
	rs.close
	...
	elseif sqlserver=TRUE then
		...
		rs.Fields("cartOrigProdID")		= IIfVs(theid<>origid,escape_string(origid))
...
function addoption(opttoadd)
	...
	if (left(opttoadd,4)="optn" OR left(opttoadd,4)="optm") AND optvalue<>"" then
		...
		elseif getpost("v"&opttoadd)="" then
			...
			sSQL="SELECT optID,"&getlangid("optGrpName",16)&","&getlangid("optName",32)&",optTxtCharge,optMultiply,optAcceptChars FROM options INNER JOIN optiongroup ON options.optGroup=optiongroup.optGrpID WHERE optID="&optvalue
			rs.open sSQL,cnn,0,1
			if NOT rs.EOF then
				...
				sSQL="INSERT INTO cartoptions (coCartID,coOptID,coOptGroup,coCartOption,coPriceDiff,coWeightDiff,coMultiply) VALUES ("&cartID&","&rs("optID")&",'"&escape_string(rs(getlangid("optGrpName",16)))&"','"&escape_string(left(theopttoadd,txtcollen))&"',"&optPriceDiff&",0," & IIfVr(rs("optMultiply")<>0,1,0) & ")"
				ect_query(sSQL)
			end if
			rs.close
		end if
...
if checkoutmode="add" then ' {
	...
	if getpost("ajaxadd")="true" then
		...
		elseif actionaftercart=4 OR cartrefreshseconds=0 then
			...
			response.redirect "cart.asp" & urllink
		else
			print "<meta http-equiv=""Refresh"" content="""&cartrefreshseconds&"; URL=cart.asp?rp="&urlencode(thefrompage)&IIfVr(getpost("PARTNER")<>"","&PARTNER="&strip_tags2(getpost("partner")),"")&listidurl&""">"
		end if
		...
		if thefrompage<>"" AND actionaftercart=3 then print thefrompage else print "cart.asp?rp="&urlencode(thefrompage)&listidurl
...
<%	function setcheckouterr(coerrmsg)
	...
	elseif NOT (paypalexpress OR amazonpayment) then
		if (enableclientlogin=TRUE OR forceclientlogin=TRUE) AND SESSION("clientID")<>"" then
		...
		if (enableclientlogin=TRUE OR forceclientlogin=TRUE) AND getpost("addressid")<>"" AND getpost("addaddress")="" AND SESSION("clientID")<>"" then
			...
			ordName=left(strip_tags2(getpost("name")),100)
			...
			ordState=left(strip_tags2(getpost("state2")),75)
			if getpost("state")<>"" then ordState=left(strip_tags2(getpost("state")),75)
		...
		if (enableclientlogin=TRUE OR forceclientlogin=TRUE) AND getpost("saddressid")<>"" AND getpost("saddaddress")="" AND SESSION("clientID")<>"" then
			...
			if (SESSION("clientID")="" AND getpost("shipdiff")="1") OR (SESSION("clientID")<>"" AND (getpost("saddaddress")="add" OR getpost("saddaddress")="edit")) then
				ordShipName=left(strip_tags2(getpost("sname")),100)
				...
				ordShipState=left(strip_tags2(getpost("sstate"&IIfVr(getpost("sstate")="","2",""))),75)
				...
				if (trim(ordShipName & ordShipLastName)="" OR ordShipAddress="" OR ordShipCity="" OR ordShipState="" OR ordShipZip="") AND trim(ordShipName & ordShipLastName & ordShipAddress & ordShipCity & ordShipState & ordShipZip & ordShipPhone & "")<>"" then
	...
	if NOT rs.EOF then
		...
		if rs("loadStates")<>-1 AND ordState="" then errordstate=setcheckouterr("You must select a state before proceeding.")
	else
		success=FALSE
	end if
	rs.close
	...
	if NOT rs.EOF then
		...
		if rs("loadStates")<>-1 AND shipstate="" AND ordShipAddress<>"" then errordshipstate=setcheckouterr("You must select a state before proceeding.")
	else
	...
	if success then
		...
		if shiphomecountry AND shipCountryID<>"" then
	...
	if ordCountry="" then
		...
		if ordPhone="" AND NOT amazonpayment then errordphone=setcheckouterr("You must enter a phone number.")
	end if
	...
	if success AND isarray(alldata) then
		...
		for index=0 to UBOUND(alldata,2)
			...
			if isnull(alldata(14,index)) AND alldata(1,index)<>giftcertificateid AND alldata(1,index)<>donationid then setcheckouterr(xxHasDel)
		next
		loyaltypointsused=0
		if success then
		...
		if (success OR getpost("shipselectoraction")="") AND NOT stockwarning then
			...
			rs.open sSQL,cnn,0,1
			do while NOT rs.EOF
				if rs("cartProdID")=giftcertificateid then ect_query("UPDATE giftcertificate SET gcOrderID="&orderid&" WHERE gcCartID="&rs("cartID"))
			...
			if getpost("remember")="1" then
				call ectsetcookie("id1",orderid,180)
				call ectsetcookie("id2",thesessionid,180)
			end if
		end if
	else
		success=FALSE
	...
	elseif ordPayProvider<>"" then
		...
		if success=FALSE then
			print "<form method=""post"" action=""cart.asp"">"
		elseif grandtotal=0 OR ordPayProvider="4" OR ordPayProvider="17" then ' Email
			print "<form method=""post"" name=""checkoutform"" action=""thanks.asp""" & IIfVs(recaptchaenabled(256)," onsubmit=""return docheckform(this)""") & ">"
		...
		elseif ordPayProvider="1" then ' PayPal
			...
			print whv("business",data1) & whv("return",storeurlssl&"thanks.asp")
			...
			print whv("cancel_return",storeurlssl&"cart.asp")
		...
		elseif ordPayProvider="2" then ' 2Checkout
			...
			print whv("purchase_step", "payment-method") & whv("currency_code", countryCurrency) & whv("x_receipt_link_url", storeurl & "thanks.asp")
		...
		elseif ordPayProvider="6" then ' NOCHEX
			...
			print whv("success_url", storeurl & "thanks.asp?ncretval="&orderid&"&ncsessid="&thesessionid) & whv("callback_url", storeurl&"vsadmin/ncconfirm.asp")
		...
		elseif ordPayProvider="7" then ' Payflow Pro
			print "<form method=""post"" action=""cart.asp"" onsubmit=""return isvalidcard(this)"">"
		...
		elseif ordPayProvider="9" then ' PayPoint.net
			...
			if requirecvv=TRUE then print whv("req_cv2", "true")
		...
		elseif ordPayProvider="11" OR ordPayProvider="12" then ' PSiGate
			...
			Session.LCID=saveLCID
			print whv("ThanksURL", storeurl&"thanks.asp") & whv("NoThanksURL", storeurl&"thanks.asp") & whv("CustomerRefNo", left(calcmd5(orderid&":"&secretword),24)) & whv("ChargeType", IIfVr(ppmethod=1, "1", "0"))
...
<%			payprovextraparams=payprovextraparams14
		elseif ordPayProvider="15" then ' Netbanx
			...
			print whv("nbx_return_url", storeurl&"categories.asp")
		...
		elseif ordPayProvider="16" then ' Linkpoint
			...
			print whv("storename", data1) & whv("mode", "payonly") & whv("ponumber", orderid) & whv("oid", orderid&sequence) & whv("responseURL", storeurl&"thanks.asp")
		...
		elseif ordPayProvider="19" then ' PayPal Express Payment
			print "<form method=""post"" action=""thanks.asp"" onsubmit=""return docheckform(this)"">"
		...
		elseif ordPayProvider="23" then ' Stripe.com
			print "<form method=""post"" action=""thanks.asp"">"
		...
		elseif ordPayProvider="24" then ' SagePay
			...
			stuff=stuff & "&Basket=" & thecart & "&SuccessURL=" & storeurl & "thanks.asp&FailureURL=" & storeurl & "thanks.asp"
	...
	if NOT returntocustomerdetails AND NOT (nopriceanywhere AND success) then ' {
		if xxCoStp3<>"" then print "<div class=""checkoutsteps"">" & xxCoStp3 & "</div>"%>
			<div class="cart3details">
			  <div class="cobhl cart3header cartheader"><%=xxChkCmp%></div>
<%		if (rgcpncode<>"" OR (ordPayProvider="19" AND getget("token")<>"")) AND (NOT gotcpncode OR cpnerror<>"") AND nogiftcertificate<>TRUE then %>
			  <div>
				...
				<div class="cobll cobll3" style="font-size:10px"><%
			if ordPayProvider="19" AND NOT gotcpncode AND getget("token")<>"" then
				print "<input type=""text"" name=""cpncode"" id=""cpncode"" size=""20"" value=""" & htmlspecials(rgcpncode) & """ autocomplete=""off"" /> <input type=""button"" value=""" & xxAppCpn & """ onclick=""document.location='cart.asp?token=" & getget("token") & "&cpncode='+document.getElementById('cpncode').value"" />"
			else
				print cpnerror
				if rgcpncode<>"" AND NOT gotcpncode then print Replace(Replace(xxNoGfCr,"%s",rgcpncode,1,1),"%s",1,1,1)
			end if %></div>
			  </div>
<%		end if
		if backorder then %>
...
function getalternatecarriers(){
	if(extraship.length>0){
		...
		ajaxobj.open("GET", "vsadmin/shipservice.asp?shiptype="+shiptype+"&numshiprate="+document.getElementById('numshiprate').value+"&sessionid=<%=urlencode(thesessionid)%>&destzip=<%=urlencode(destZip)%>&sc=<%=urlencode(shipcountry)%>&scc=<%=urlencode(shipCountryCode)%>&sta=<%=urlencode(shipStateAbbrev)%>&orderid=<%=orderid%>", true);
		ajaxobj.send(null);
	}
}
<%		end if
...
<%		if shipType=0 then combineshippinghandling=FALSE
		if shipType<>0 then
			...
			doshowlogo=(NOT ((shipType=1 OR shipType=2 OR shipType=5) AND shippinglogo="" AND adminAltRates=0))
%>			  <div class="cart3row">
			...
			print "<div class=""shipoptionstable"" id=""shipoptionstable""><div class=""shiptableline""><div class=""shiptablelogo"">" & getshiplogo(currShipType) & "</div><div class=""shiptablerates" & IIfVs(NOT success," ectwarning") & """>"
			if NOT success then
				print errormsg
			else
...
<%		end if
		if adminAltRates=1 then
			...
			if NOT rs.EOF then %>
			  <div class="cart3row">
			    <div class="cobhl cobhl3"><%=xxOrCom%></div>
				<div class="cobll cobll3"><%
				if shippingoptionsasradios<>TRUE then print "<select id=""altratesselect"" size=""1"" onchange=""selaltrate(this[this.selectedIndex].value)"">"
				do while NOT rs.EOF
					call writealtshipline(rs(getlangid("altratetext",65536)),rs("altrateid"),"","")
					rs.movenext
				loop
				if shippingoptionsasradios<>TRUE then print "</select>"
%>				</div>
...
<%		end if %>
			  <div class="cart3row">
			    <div class="cobhl cobhl3 cart3gndtot"><%=xxGndTot%></div>
				<div class="cobll cobll3 cart3gndtott"><% if success=FALSE then print "-" else print FormatEuroCurrency(grandtotal)%></div>
			  </div>
<%		if NOT (ordPayProvider="7" OR ordPayProvider="13" OR ordPayProvider="18") then cardinalprocessor=""
		checkcarddetails=FALSE
		if success AND grandtotal>0 AND (ordPayProvider="7" OR ordPayProvider="10" OR ordPayProvider="12" OR ordPayProvider="13" OR (ordPayProvider="14" AND customppacceptcc) OR (ordPayProvider="16" AND (data2&"")="1") OR ordPayProvider="18") then ' { Payflow Pro OR PSiGate SSL OR Auth.NET AIM OR PayPal Pro
			...
			acceptecheck=acceptecheck=TRUE AND (ordPayProvider="13" OR (customppacceptecheck AND ordPayProvider="14"))
%>
...
<%			end if %>
			  ...
			  <div class="cart4row">
				...
				<div class="cobll cobll4"><input type="text" name="<%=sscardname%>" name="<%=sscardname%>" size="21" value="<%=trim(ordName&" "&ordLastName)%>" AUTOCOMPLETE="off" /></div>
			  </div>
			  <div class="cart4row">
			    <div class="cobhl cobhl4"><%=labeltxt(xxCrdNum,sscardnum)%></div>
				<div class="cobll cobll4"><input type="text" name="<%=sscardnum%>" id="<%=sscardnum%>" size="21" AUTOCOMPLETE="off" />
<%				if cardinalprocessor<>"" AND cardinalmerchant<>"" AND cardinalpwd<>"" then %>
...
<%				end if %>
			  ...
			  <div class="cart4row">
				...
				<div class="cobll cobll4">
				  <select name="<%=ssexmon%>" id="<%=ssexmon%>" size="1">
					...
					<%	for index=1 to 12
						...
						next %>
				  </select> / <select name="<%=ssexyear%>" id="<%=ssexyear%>" size="1">
					<option value=""><%=xxYear%></option>
					<%	thisyear=DatePart("yyyy", Date())
						for index=thisyear to thisyear+10
							print "<option value='"&IIfVr(isPSiGate,right(index,2),index)&"'>"&index&"</option>"&vbCrLf
						next %></select>
				</div>
			  </div>
			  <div class="cart4row">
			    <div class="cobhl cobhl4"><%=labeltxt(xx34code,sscvv2)%></div>
				<div class="cobll cobll4"><input type="text" name="<%=sscvv2%>" id="<%=sscvv2%>" size="4" AUTOCOMPLETE="off" /> <%if requirecvv<>TRUE then print xxIfPres%></div>
			  </div>
<%			if Mid(data1,8,1)="X" then %>
			  <div class="cart4row">
...
<%				end if
		...
		if success then %>
			  <div class="cart4row">
			    <div class="cobhl cart2column"><% if cardinalprocessor<>"" AND cardinalmerchant<>"" AND cardinalpwd<>"" then print xxCentl else print xxMstClk%></div>
			  </div>
			  <div class="cobll cart2column checkoutbutton3"><%
			if (checkcarddetails AND recaptchaenabled(1)) OR ((ordPayProvider="4" OR ordPayProvider="17") AND recaptchaenabled(256)) then
				call displayrecaptchajs("cardentrycaptcha",TRUE,FALSE)
				print "<div id=""cardentrycaptcha"" class=""g-recaptcha reCAPTCHAcheckout""></div>"
			end if
			if orderid<>0 then
				if ordPayProvider="23" AND grandtotal>0 then
					print "<script src=""https://checkout.stripe.com/checkout.js"" class=""stripe-button"" data-key="""&data2&""" data-amount="""&round(grandtotal*100)&""" data-currency="""&countryCurrency&""" data-email="""&ordEmail&""" " & IIfVs(data3<>"","data-name="""&data3&""" ") & "data-description=""" & left(htmlspecials(descstr),255) & """ data-image=""/128x128.png""></script>"
				else
					print imageorsubmit(imgcheckoutbutton3,IIfVr(xxCOTxt3<>"",xxCOTxt3,xxCOTxt)&IIfVs(closeorderimmediately,""" onclick=""docloseorder()"),"checkoutbutton checkoutbutton3")
				end if
			end if %>
			  </div>
<%		end if %>
...
<%		end if
		print "</form>" & vbCrLf & "<form method=""post"" name=""shipform"" id=""shipform"" action=""cart.asp"">"
		...
		if NOT fromshipselector AND adminAltRates=2 then print "<script type=""text/javascript"">getalternatecarriers();</script>"
	end if ' }
	if nopriceanywhere AND success then
		...
		print "</form>"
		if NOT recaptchaenabled(256) then print "<script type=""text/javascript"">document.checkoutform.submit();</script>"
	end if
...
if checkoutmode="checkout" then ' }{
	...
	Dim remember,havestate,allcountries
	sSQL="SELECT ordID FROM orders WHERE ordStatus>1 AND ordAuthNumber='' AND " & getordersessionsql()
	rs.open sSQL,cnn,0,1
	if NOT rs.EOF then ordID=rs("ordID") else ordID=""
	rs.close
		...
		if NOT rs.EOF then
			...
			if NOT rs.EOF then
				...
				if rs("clEmail")<>request.cookies("WRITECLL") OR rs("clPW")<>request.cookies("WRITECLP") then
					call ectsetcookie("WRITECLL",rs("clEmail"),0)
					call ectsetcookie("WRITECLP",rs("clPW"),0)
				end if
			end if
		end if
		rs.close
	end if
	if request.cookies("id2")<>"" AND is_numeric(request.cookies("id1")) AND NOT returntocustomerdetails then
		call retrieveorderdetails(request.cookies("id1"),request.cookies("id2"))
		remember=TRUE
	end if
...
<%		for ii=0 to UBOUND(addresses,2)
			print "addrs[" & addresses(0,ii) & "]=new Array();" & vbCrLf
			print "addrs[" & addresses(0,ii) & "]['name']='" & jschk(addresses(2,ii)) & "';"
			print "addrs[" & addresses(0,ii) & "]['lastname']='" & jschk(addresses(12,ii)) & "';"
			print "addrs[" & addresses(0,ii) & "]['address']='" & jschk(addresses(3,ii)) & "';"
			print "addrs[" & addresses(0,ii) & "]['address2']='" & jschk(addresses(4,ii)) & "';"
			print "addrs[" & addresses(0,ii) & "]['state']='" & jschk(addresses(5,ii)) & "';"
			print "addrs[" & addresses(0,ii) & "]['city']='" & jschk(addresses(6,ii)) & "';"
			print "addrs[" & addresses(0,ii) & "]['zip']='" & jschk(addresses(7,ii)) & "';"
			print "addrs[" & addresses(0,ii) & "]['phone']='" & jschk(addresses(8,ii)) & "';"
			print "addrs[" & addresses(0,ii) & "]['country']='" & jschk(addresses(9,ii)) & "';"
			print "addrs[" & addresses(0,ii) & "]['countryid']=" & getidfromcountry(addresses(9,ii)) & ";"
			print "addrs[" & addresses(0,ii) & "]['extra1']='" & jschk(addresses(10,ii)) & "';"
			print "addrs[" & addresses(0,ii) & "]['extra2']='" & jschk(addresses(11,ii)) & "';" & vbCrLf
		next %>
/* ]]> */</script>
<%	end if
	print whv("mode","go")
...
<% if xxCoStp2<>"" then print "<div class=""checkoutsteps"">" & xxCoStp2 & "</div>"%>
		<div class="cart2details">
			<div class="cobhl cart2header cartheader"><%=xxCstDtl%></div>
<%	if returntocustomerdetails AND errormsg<>"" then print "<div class=""cobhl cart2column ectwarning"">" & errormsg & "</div>" & vbCrLf
	if getpayprovdetails(21,data1,data2,data3,demomode,ppmethod) then
		...
		if xxAmzPBT="" then xxAmzPBT="- or enter your customer and payment details below -"
		print "<div class=""amazonpaybuttontext"">" & xxAmzPBT & "</div>"
...
var authRequest;
OffAmazonPayments.Button("AmazonPayButton", "<%=data2%>", { // MERCHANT_ID
	type: "<%=IIfVr(SESSION("AmazonLogin")<>"","PwA","LwA")%>",
	authorization: function () {
		loginOptions = { scope: "profile postal_code payments:widget payments:shipping_address", popup: true };
		authRequest = amazon.Login.authorize(loginOptions, "<%=storeurlssl & "cart.asp?amazonpay=go"%>");
	},
...
function amazonlogout(){
	amazon.Login.logout();
	document.location='cart.asp?amazon=logout';
	return false;
}
</script>
<%	end if
	if isarray(addresses) then %>
				<div>
				  <div class="cdformtitle"><%=xxBilAdd%></div>
				  <div class="cdformtitlell">
<%		sub writeaddressspans(isshp) %>
		<span id="<%=isshp%>addressspan1" style="display:block"><select name="<%=isshp%>addressid" id="<%=isshp%>addressid" size="1" onchange="checkeditbutton('<%=isshp%>')"><%
		...
		next %></select> <input type="button" value="<%=xxEdit%>" id="<%=isshp%>editbutton" onclick="editaddress('<%=isshp%>',false);document.getElementById('<%=isshp%>addressspan1').style.display='none';document.getElementById('<%=isshp%>addressspan2').style.display='block';document.getElementById('<%=isshp%>addaddress').value='edit';"> <input type="button" value="<%=xxNew%>" onclick="editaddress('<%=isshp%>',true);document.getElementById('<%=isshp%>addressspan1').style.display='none';document.getElementById('<%=isshp%>addressspan2').style.display='block';document.getElementById('<%=isshp%>addaddress').value='add';">
		</span><div id="<%=isshp%>addressspan2" style="display:none">
			<%	if trim(extraorderfield1)<>"" then %>
			<div class="billformrow"><div class="cobhl cobhl2 cdformtextra1"><%=IIfVr(extraorderfield1required=true,redstar,"") & labeltxt(extraorderfield1,"ord"&isshp&"extra1")%>:</div><div class="cobll cobll2 cdformextra1"><% if extraorderfield1html<>"" then print replace(replace(extraorderfield1html,"ectfield","ord"&isshp&"extra1"),"ordextra1","ord"&isshp&"extra1") else print "<input type=""text"" name=""ord"&isshp&"extra1"" id=""ord"&isshp&"extra1"" size=""20"" autocomplete=""false"" />"%></div></div>
			<%	end if %>
			<div class="billformrow"><div class="cobhl cobhl2 cdformtname<%=IIfVs(isshp="" AND errordname," ectwarning")%>"><%=redstar & labeltxt(xxName,isshp&"name")%>:</div><div class="cobll cobll2 cdformname"><%
			if usefirstlastname then
				print "<input type=""text"" name="""&isshp&"name"" autocomplete=""given-name"" id="""&isshp&"name"" size=""11"" placeholder="""&htmlspecials(xxFirNam)&""" /> <input type=""text"" name="""&isshp&"lastname"" autocomplete=""family-name"" id="""&isshp&"lastname"" size=""11"" placeholder="""&htmlspecials(xxLasNam)&""" />"
			else
				print "<input type=""text"" name="""&isshp&"name"" id="""&isshp&"name"" size=""20"" />"
			end if
			%></div></div>
			<div class="billformrow"><div class="cobhl cobhl2 cdformtaddress<%=IIfVs(isshp="" AND errordaddress," ectwarning")%>"><%=redstar & labeltxt(xxAddress,isshp&"address")%>:</div><div class="cobll cobll2 cdformaddress"><input type="text" name="<%=isshp%>address" id="<%=isshp%>address" size="25" /></div></div>
			<%	if useaddressline2=TRUE then %>
			<div class="billformrow"><div class="cobhl cobhl2 cdformtaddress2"><%=labeltxt(xxAddress2,isshp&"address2")%>:</div><div class="cobll cobll2 cdformaddress2"><input type="text" name="<%=isshp%>address2" id="<%=isshp%>address2" size="25" /></div></div>
			<%	end if %>
			<div class="billformrow"><div class="cobhl cobhl2 cdformtcity<%=IIfVs(isshp="" AND errordcity," ectwarning")%>"><%=redstar & labeltxt(xxCity,isshp&"city")%>:</div><div class="cobll cobll2 cdformcity"><input type="text" name="<%=isshp%>city" id="<%=isshp%>city" size="20" /></div></div>
			<div class="billformrow"><div class="cobhl cobhl2 cdformtstate<%=IIfVs((isshp="s" AND errordshipstate) OR (isshp<>"s" AND errordstate)," ectwarning")%>"><%=replace(redstar,"<span","<span id="""&isshp&"statestar""")%><span id="<%=isshp%>statetxt"><%=labeltxt(xxState,isshp&"state")%></span>:</div><div class="cobll cobll2 cdformstate"><% if hasstates then %><select name="<%=isshp%>state" id="<%=isshp%>state" size="1" onchange="dosavestate('')"><% havestate=show_states(-1) %></select><% end if %><input type="text" name="<%=isshp%>state2" id="<%=isshp%>state2" size="20" /></div></div>
			<div><div class="cobhl cobhl2 cdformtcountry<%=IIfVs(isshp="" AND errordcountry," ectwarning")%>"><%=redstar & labeltxt(xxCountry,isshp&"country")%>:</div><div class="cobll cobll2 cdformcountry"><select name="<%=isshp%>country" id="<%=isshp%>country" size="1" onchange="checkoutspan('<%=isshp%>');showshipform(1,this)"><% call show_countries(-1,TRUE) %></select></div></div>
			<div class="billformrow"><div class="cobhl cobhl2 cdformtzip<%=IIfVs(isshp="" AND errordzip," ectwarning")%>"><%=replace(redstar,"<span","<span id="""&isshp&"zipstar""") & labeltxt(xxZip,isshp&"zip")%>:</div><div class="cobll cobll2 cdformzip"><input type="text" name="<%=isshp%>zip" id="<%=isshp%>zip" size="10" autocapitalize="characters" /></div></div>
			<div class="billformrow"><div class="cobhl cobhl2 cdformtphone<%=IIfVs(isshp="" AND errordphone," ectwarning")%>"><% if isshp="" then print redstar %><%=labeltxt(xxPhone,isshp&"phone")%>:</div><div class="cobll cobll2 cdformphone"><input type="tel" name="<%=isshp%>phone" id="<%=isshp%>phone" size="20" /></div></div>
			...
			<%	end if %>
			<div class="cobll cdform2column"><input type="button" value="<%=xxCancel%>" onclick="document.getElementById('<%=isshp%>addressspan2').style.display='none';document.getElementById('<%=isshp%>addressspan1').style.display='block';document.getElementById('<%=isshp%>addaddress').value='';<%=isshp%>checkaddress=false;"></div>
		</div>
<%		end sub
...
<%		call writeshippingflags(2)
		if noshipaddress<>TRUE then %>
				<div>
				  <div class="cdformtitle"><%=xxShpAdd%></div>
...
<%		end if
		...
		function displayzip(isship) %>
				<div class="<%=IIfVr(isship="s","ship","bill")%>formrow">
				  <div class="cobhl cobhl2 cdformtzip<%=IIfVs((isship="" AND errordzip) OR (isship="s" AND errordshipaddress AND ordShipZip="")," ectwarning")%>"><%=replace(redstar,"<span","<span id="""&isship&"zipstar""") %><span id="<%=isship%>ziptxt"><%=labeltxt(xxZip,isship&"zip")%></span></div>
				  <div class="cobll cobll2 cdformzip"><input type="text" name="<%=isship%>zip" class="cdforminput cdformzip" id="<%=isship%>zip" size="10" value="<%=htmlspecials(IIfVr(isship="s",ordShipZip,ordZip))%>" autocapitalize="characters" /></div>
				</div>
<%		end function
		if trim(extraorderfield1)<>"" then %>
				<div class="billformrow" style="display:none">
				  ...
				  <div class="cobll cobll2 cdformextra1"><% if extraorderfield1html<>"" then print replace(extraorderfield1html,"ectfield","ordextra1") else print "<input type=""text"" name=""ordextra1"" class=""cdforminput cdformextra1"" id=""ordextra1"" size=""20"" value="""&htmlspecials(ordExtra1)&""" autocomplete=""false"" />"%></div>
				</div>
<%		end if %>
				<div class="billformrow" style="display:none">
				  <div class="cobhl cobhl2 cdformtname<%=IIfVs(errordname," ectwarning")%>"><%=redstar & labeltxt(xxName,"name")%></div>
				  <div class="cobll cobll2 cdformname" style="white-space:nowrap"><%
		if usefirstlastname then
			print "<input type=""text"" name=""name"" autocomplete=""given-name"" class=""cdforminput cdformname"" id=""name"" size=""10"" value="""&htmlspecials(ordName)&""" placeholder="""&htmlspecials(xxFirNam)&""" /> <input type=""text"" name=""lastname"" autocomplete=""family-name"" class=""cdforminput cdformlastname"" size=""10"" value="""&htmlspecials(ordLastName)&""" placeholder="""&htmlspecials(xxLasNam)&""" />"
		else
			print "<input type=""text"" name=""name"" class=""cdforminput cdformname"" id=""name"" size=""20"" value="""&htmlspecials(ordName)&""" />"
		end if %></div>
				</div>
				<div class="billformrow" style="display:none">
				  <div class="cobhl cobhl2 cdformtemail<%=IIfVs(errordemail," ectwarning")%>"><%=redstar & labeltxt(xxEmail,"email")%></div>
				  <div class="cobll cobll2 cdformemail"><input type="email" name="email" class="cdforminput cdformemail" id="email" size="25" value="<%=htmlspecials(ordEmail)%>" /></div>
				</div>
				<div class="billformrow" style="display:none">
				  <div class="cobhl cobhl2 cdformtaddress<%=IIfVs(errordaddress," ectwarning")%>"><%=redstar & labeltxt(xxAddress,"address")%></div>
				  <div class="cobll cobll2 cdformaddress"><input type="text" name="address" class="cdforminput cdformaddress" id="address" size="25" value="<%=htmlspecials(ordAddress)%>" /></div>
				</div>
<%		if useaddressline2=TRUE then %>
				<div class="billformrow" style="display:none">
				  <div class="cobhl cobhl2 cdformtaddress2"><%=labeltxt(xxAddress2,"address2")%></div>
				  <div class="cobll cobll2 cdformaddress2"><input type="text" name="address2" class="cdforminput cdformaddress2" id="address2" size="25" value="<%=htmlspecials(ordAddress2)%>" /></div>
				</div>
<%		end if
		if zipposition=4 then call displayzip("") %>
				<div class="billformrow" style="display:none">
				  ...
				  <div class="cobll cobll2 cdformcity"><input type="text" name="city" class="cdforminput cdformcity" id="city" size="20" value="<%=htmlspecials(ordCity)%>" /></div>
				</div>
<%		if zipposition=3 then call displayzip("") %>
				<div class="billformrow" style="display:none">
				  <div class="cobhl cobhl2 cdformtstate<%=IIfVs(errordstate," ectwarning")%>"><%=replace(redstar,"<span","<span id=""statestar""")%><span id="statetxt"><%=labeltxt(xxState,"state2")%></span></div>
				  <div class="cobll cobll2 cdformstate"><% if hasstates then %><select name="state" class="cdformselect cdformstate" id="state" size="1" onchange="dosavestate('')"><% havestate=show_states(ordState) %></select><% end if %><input type="text" name="state2" class="cdforminput cdformstate" id="state2" style="display:none" size="20" value="<% if not havestate then print htmlspecials(ordState)%>" /></div>
				</div>
<%		if zipposition=2 then call displayzip("") %>
				<div>
				  ...
				  <div class="cobll cobll2 cdformcountry"><select name="country" class="cdformselect cdformcountry" id="country" size="1" onchange="checkoutspan('');showshipform(1,this)" ><% call show_countries(ordCountry,TRUE) %></select></div>
				</div>
<%		if zipposition=1 then call displayzip("") %>
				<div class="billformrow" style="display:none">
				  ...
				  <div class="cobll cobll2 cdformphone"><input type="tel" name="phone" class="cdforminput cdformphone" id="phone" size="20" value="<%=htmlspecials(ordPhone)%>" /></div>
				</div>
<%		if trim(extraorderfield2)<>"" then %>
				<div class="billformrow" style="display:none">
				  ...
				  <div class="cobll cobll2 cdformextra2"><% if extraorderfield2html<>"" then print replace(extraorderfield2html,"ectfield","ordextra2") else print "<input type=""text"" name=""ordextra2"" class=""cdforminput cdformextra2"" id=""ordextra2"" size=""20"" value="""&ordExtra2&""" autocomplete=""false"" />"%></div>
				</div>
<%		end if
		call writeshippingflags(2)
		if noshipaddress<>TRUE then %>
				<div>
				  <div class="cobhl cobhl2 cdformshipdiff">
					<input name="shipdiff" class="cdformcb cdformshipdiff" id="shipdiff" value="1" type="checkbox" onclick="checkoutspan('s');showshipform(2,document.getElementById('scountry'))" <%=IIfVs(getpost("shipdiff")="1" OR (trim(ordShipName&ordShipLastName)<>"" AND trim(ordShipAddress)<>""),"checked=""checked"" ")%>/><%=xxShpDff%>
				  </div>
				</div>
		<%	if trim(extraorderfield1)<>"" then %>
				<div class="shipformrow" style="display:none">
				  ...
				  <div class="cobll cobll2 cdformextra1"><% if extraorderfield1html<>"" then print replace(replace(extraorderfield1html,"ordextra1","ordsextra1"),"ectfield","ordsextra1") else print "<input type=""text"" name=""ordsextra1"" class=""cdforminput cdformextra1"" id=""ordsextra1"" size=""20"" value="""&htmlspecials(ordShipExtra1)&""" autocomplete=""false"" />"%></div>
				</div>
<%			end if %>
				<div class="shipformrow" style="display:none">
				  <div class="cobhl cobhl2 cdformtname<%=IIfVs(errordshipaddress AND ordShipName=""," ectwarning")%>"><%=redstar & labeltxt(xxName,"sname")%></div>
				  <div class="cobll cobll2 cdformname"><%
			if usefirstlastname then
				print "<input type=""text"" name=""sname"" class=""cdforminput cdformname"" id=""sname"" size=""10"" value="""&htmlspecials(ordShipName)&""" placeholder="""&htmlspecials(xxFirNam)&""" /> <input type=""text"" name=""slastname"" class=""cdforminput cdformlastname"" id=""slastname"" size=""10"" value="""&htmlspecials(ordShipLastName)&""" placeholder="""&htmlspecials(xxLasNam)&""" />"
			else
				print "<input type=""text"" name=""sname"" class=""cdforminput cdformname"" id=""sname"" size=""20"" value="""&htmlspecials(ordShipName)&""" />"
			end if %></div>
				</div>
				<div class="shipformrow" style="display:none">
				  ...
				  <div class="cobll cobll2 cdformaddress"><input type="text" name="saddress" class="cdforminput cdformaddress" id="saddress" size="25" value="<%=htmlspecials(ordShipAddress)%>" /></div>
				</div>
<%			if useaddressline2=TRUE then %>
				<div class="shipformrow" style="display:none">
				  <div class="cobhl cobhl2 cdformtaddress2"><%=labeltxt(xxAddress2,"saddress2")%></div>
				  <div class="cobll cobll2 cdformaddress2"><input type="text" name="saddress2" class="cdforminput cdformaddress2" id="saddress2" size="25" value="<%=htmlspecials(ordShipAddress2)%>" /></div>
				</div>
<%			end if
			if zipposition=4 then call displayzip("s") %>
				<div class="shipformrow" style="display:none">
				  ...
				  <div class="cobll cobll2 cdformcity"><input type="text" name="scity" class="cdforminput cdformcity" id="scity" size="20" value="<%=htmlspecials(ordShipCity)%>" /></div>
				</div>
<%			if zipposition=2 then call displayzip("s") %>
				<div class="shipformrow" style="display:none">
				  ...
				  <div class="cobll cobll2 cdformcountry"><select name="scountry" class="cdformselect cdformcountry" id="scountry" size="1" onchange="checkoutspan('s')"><option value=""><%=xxPlsSel%>...</option><% call show_countries(ordShipCountry,FALSE) %></select></div>
				</div>
<%			if zipposition=3 then call displayzip("s") %>
				<div class="shipformrow" style="display:none">
				  <div class="cobhl cobhl2 cdformtstate<%=IIfVs(errordshipstate," ectwarning")%>"><%=replace(redstar,"<span","<span id=""sstatestar""")%><span id="sstatetxt"><%=labeltxt(xxState,"sstate2")%></span></div>
				  <div class="cobll cobll2 cdformstate"><% if hasstates then %><select name="sstate" class="cdformselect cdformstate" id="sstate" size="1" onchange="dosavestate('s')"><% havestate=show_states(ordShipState) %></select><% end if %><input type="text" name="sstate2" class="cdforminput cdformstate" id="sstate2" style="display:none" size="20" value="<% if not havestate then print htmlspecials(ordShipState)%>" /></div>
				</div>
<%			if zipposition=1 then call displayzip("s") %>
				<div class="shipformrow" style="display:none">
				  <div class="cobhl cobhl2 cdformtphone"><%=labeltxt(xxPhone,"sphone")%></div>
				  <div class="cobll cobll2 cdformphone"><input type="tel" name="sphone" class="cdforminput cdformphone" id="sphone" size="20" value="<%=htmlspecials(ordShipPhone)%>" /></div>
				</div>
<%			if trim(extraorderfield2)<>"" then %>
				<div class="shipformrow" style="display:none">
				  <div class="cobhl cobhl2 cdformtextra2"><% if extraorderfield2required=TRUE then print redstar
									print labeltxt(extraorderfield2,"ordsextra2")%></div>
				  <div class="cobll cobll2 cdformextra2"><% if extraorderfield2html<>"" then print replace(replace(extraorderfield2html,"ordextra2","ordsextra2"),"ectfield","ordsextra2") else print "<input type=""text"" name=""ordsextra2"" class=""cdforminput cdformextra2"" id=""ordsextra2"" size=""20"" value="""&ordShipExtra2&""" autocomplete=""false"" />"%></div>
				</div>
<%			end if
...
<%	if noadditionalinfo<>TRUE then %>
				<div class="checkoutadditionals">
				  ...
				  <div class="cobll cobll2 cdformaddinfo"><textarea name="ordAddInfo" class="addinfo" rows="3" cols="44" autocomplete="false"><%=htmlspecials(ordAddInfo)%></textarea></div>
				</div>
<%	end if
	if trim(extracheckoutfield1)<>"" then
		...
		checkoutfield2=IIfVr(extracheckoutfield1html<>"", replace(extracheckoutfield1html,"ectfield","ordcheckoutextra1"), "<input type=""text"" name=""ordcheckoutextra1"" class=""cdforminput cdformextraco1"" id=""ordcheckoutextra1"" size=""20"" value="""&htmlspecials(ordCheckoutExtra1)&""" autocomplete=""false"" />")
%>				<div class="checkoutadditionals">
...
<%	end if
	if trim(extracheckoutfield2)<>"" then
		...
		checkoutfield2=IIfVr(extracheckoutfield2html<>"", replace(extracheckoutfield2html,"ectfield","ordcheckoutextra2"), "<input type=""text"" name=""ordcheckoutextra2"" class=""cdforminput cdformextraco2"" id=""ordcheckoutextra2"" size=""20"" value="""&htmlspecials(ordCheckoutExtra2)&""" autocomplete=""false"" />")
%>				<div class="checkoutadditionals">
...
<%	end if
	if termsandconditions then %>
				<div class="checkoutadditionals">
				  <div class="cobhl cobhl2 cdformtterms"><input type="checkbox" name="license" class="cdformcb cdformterms" value="1" /></div>
				  <div class="cobll cobll2 cdformterms<%=IIfVs(errtermsandconditions," ectwarning")%>"><%=xxTermsCo%></div>
				</div>
<%	end if
	if SESSION("clientID")="" AND noremember<>TRUE then %>
				<div class="checkoutadditionals">
				  <div class="cobhl cobhl2 cdformtremember"><input type="checkbox" name="remember" class="cdformcb cdformremember" value="1" <% if remember then print "checked=""checked"""%> /></div>
				  <div class="cobll cobll2 cdformremember"><%=xxRemMe%><br /><span class="ectsmallnote"><%=xxOpCook%></span></div>
				</div>
<%	end if
	if nomailinglist<>TRUE then %>
				<div class="checkoutadditionals">
				  <div class="cobhl cobhl2 cdformtmailing"><input type="checkbox" name="allowemail" class="cdformcb cdformmailing" value="ON" <% if allowemaildefaulton=TRUE then print "checked=""checked"""%> /></div>
				  <div class="cobll cobll2 cdformmailing"><%=xxAlPrEm%><br /><span class="ectsmallnote"><%=xxNevDiv%></span></div>
				</div>
<%	end if
	if nogiftcertificate<>TRUE then %>
				<div><div class="cobhl cobhl2 cdformtcoupon"><%=labeltxt(xxGifNum,"cpncode")%></div><div class="cobll cobll2 cdformcoupon">
			<div><input type="text" name="cpncode" class="cdforminput cdformcoupon" id="cpncode" size="<%=IIfVr(mobilebrowser,14,20)%>" autocomplete="off" /> <%=imageorbutton(imgapplycoupon,xxApply,"applycoupon applycoupon2","applycert()",TRUE)%></div>
		<div id="cpncodespan"><%
...
<%	elseif UBOUND(alldata,2)=0 then
		...
		payproviderradios=""
				...
				<div><div class="cobhl cobhl2 cdformtpayment"><%=xxPlsChz%></div>
				  <div class="cobll cobll2 cdformpayment"><%
		if payproviderradios=1 OR payproviderradios=2 then
			print "<div class=""payprovider"">"
			for rowcounter=0 to UBOUND(alldata,2)
				print IIfVs(payproviderradios=1,"<div class=""payprovline"">")&"<input type=""radio"" name=""payprovider"" class=""cdformradio cdformpayment"" value="""&alldata(0,rowcounter)&""""
				...
				print " />"&alldata(1,rowcounter)&IIfVs(alldata(0,rowcounter)=1,"<img src=""images/paypalacceptmark.gif"" alt=""PayPal Payments"" />") & IIfVs(payproviderradios=1,"</div>")
			next
			print "</div>"
		else
			...
			for rowcounter=0 to UBOUND(alldata,2)
				...
				if ordPayProvider=alldata(0,rowcounter) then print " selected=""selected"""
...
<%	end if
	if ppsuccess then %>
				<div>
			      <div class="cobll cart2column checkoutbutton2"><%=imageorsubmit(imgcheckoutbutton2,IIfVr(xxCOTxt2<>"",xxCoTxt2,xxCOTxt),"checkoutbutton checkoutbutton2")%></div>
				</div><%
	end if %> </div>
			</form>
<script type="text/javascript">
...
function chkextra(isship,ob,fldtxt){
	...
	if(ob){
		...
		if(!hasselected){
			...
			if(isship){if(!confirm("<%=jscheck(xxPlsEntr&" """&xxShpDet & " / ")%>"+fldtxt+"\".\n\n<%=jscheck(xxNoShip)%>"))unselectshipadd();
			}else alert("<%=jscheck(xxPlsEntr)%> \""+fldtxt+"\".");
			return(false);
		}
	}else{
...
function checkform(frm){
	...
	shipaddcan=false;
if(checkaddress){
if(frm.country[frm.country.selectedIndex].value=='') return(chkfocus(frm.country,"<%=jscheck(xxPlsSlct & " " & xxCountry)%>"));
<%	if trim(extraorderfield1)<>"" AND extraorderfield1required=true then print "if(!chkextra(false,frm.ordextra1,"""&jscheck(strip_tags2(extraorderfield1))&"""))return(false);" &vbCrLf %>
if(frm.name.value==""||frm.name.value=="<%=jscheck(xxFirNam)%>") return(chkfocus(frm.name,"<%=jscheck(xxPlsEntr&" """&IIfVr(usefirstlastname, xxFirNam, xxName))%>\"."));
<%	if usefirstlastname=TRUE then %>
if(frm.lastname.value=="") return(chkfocus(frm.lastname,"<%=jscheck(xxPlsEntr&" """&xxLasNam)%>\"."));
<%	else %>
var regex=/ /;
...
<%	end if %>
if(frm.address.value=="") return(chkfocus(frm.address,"<%=jscheck(xxPlsEntr&" """&xxAddress)%>\"."));
if(frm.city.value=="") return(chkfocus(frm.city,"<%=jscheck(xxPlsEntr&" """&xxCity)%>\"."));
	if(stateoptional(cntelem)){
	}else if(stateselectordisabled[0]==false){
<%	if hasstates then %>
		if(frm.state.selectedIndex==0) return(chkfocus(frm.state,"<%=jscheck(xxPlsSlct & " ")%>"+document.getElementById('statetxt').innerHTML));
<%	end if %>
	}else if(frm.state2.value=="")
		return(chkfocus(frm.state2,"<%=jscheck(xxPlsEntr)%> \""+document.getElementById('statetxt').innerHTML+"\"."));
if(frm.zip.value==""&&!zipoptional(cntelem)) return(chkfocus(frm.zip,"<%=jscheck(xxPlsEntr)%> \""+getziptext(cntelem[cntelem.selectedIndex].value)+"\"."));
if(frm.phone.value=="") return(chkfocus(frm.phone,"<%=jscheck(xxPlsEntr&" """&xxPhone)%>\"."));
<%	if trim(extraorderfield2)<>"" AND extraorderfield2required=true then print "if(!chkextra(false,frm.ordextra2,"""&jscheck(strip_tags2(extraorderfield2))&"""))return(false);" &vbCrLf %>
}
<% if abs(addshippinginsurance)=2 AND forceinsuranceselection then %>
if(frm.wantinsurance.selectedIndex==0){
	alert("<%=jscheck(strip_tags2(replace(xxChoIns,"<br />","\n")))%>");
...
if(scheckaddress&&document.getElementById('shipdiff').checked){
	if(frm.scountry[frm.scountry.selectedIndex].value==''&&!chkconfship(frm.country,"<%=jscheck(xxPlsSlct&" """&xxShpDet&" / "&xxCountry)%>\"."+xxnoship)) return false;
<%	if trim(extraorderfield1)<>"" AND extraorderfield1required=true then print "if(!chkextra(true,frm.ordsextra1,"""&jscheck(strip_tags2(extraorderfield1))&"""))return(false);" &vbCrLf %>
	if(!shipaddcan&&(frm.sname.value==""||frm.sname.value=="<%=xxFirNam%>")&&!chkconfship(frm.sname,"<%=jscheck(xxPlsEntr&" """&xxShpDet&" / "&xxName)%>\"."+xxnoship)) return false;
	if(!shipaddcan&&frm.saddress.value==""&&!chkconfship(frm.saddress,"<%=jscheck(xxPlsEntr&" """&xxShpDet&" / "&xxAddress)%>\"."+xxnoship)) return false;
	if(!shipaddcan&&frm.scity.value==""&&!chkconfship(frm.scity,"<%=jscheck(xxPlsEntr&" """&xxShpDet&" / "&xxCity)%>\"."+xxnoship)) return false;
	if(stateoptional(scntelem)){
	}else if(stateselectordisabled[1]==false){
<%	if hasstates then %>
		if(!shipaddcan&&frm.sstate.selectedIndex==0&&!chkconfship(frm.sstate,"<%=jscheck(xxPlsSlct&" """&xxShpDet&" / ")%>"+document.getElementById('sstatetxt').innerHTML+"\"."+xxnoship)) return false;
<%	end if %>
	}else if(!shipaddcan&&frm.sstate2.value==""&&!chkconfship(frm.sstate2,"<%=jscheck(xxPlsEntr&" """&xxShpDet&" / ")%>"+document.getElementById('sstatetxt').innerHTML+"\"."+xxnoship))
		return false;
	if(!shipaddcan&&frm.szip.value==""&&!zipoptional(scntelem)&&!chkconfship(frm.szip,"<%=jscheck(xxPlsEntr&" """&xxShpDet&" / ")%>"+getziptext(scntelem[scntelem.selectedIndex].value)+"\"."+xxnoship)) return false;
<%	if trim(extraorderfield2)<>"" AND extraorderfield2required=true then print "if(!chkextra(true,frm.ordsextra2,"""&jscheck(strip_tags2(extraorderfield2))&"""))return(false);" &vbCrLf %>
}
<% end if
	if trim(extracheckoutfield1)<>"" AND extracheckoutfield1required=true then print "if(!chkextra(false,frm.ordcheckoutextra1,"""&jscheck(strip_tags2(extracheckoutfield1))&"""))return(false);" &vbCrLf
	if trim(extracheckoutfield2)<>"" AND extracheckoutfield2required=true then print "if(!chkextra(false,frm.ordcheckoutextra2,"""&jscheck(strip_tags2(extracheckoutfield2))&"""))return(false);" &vbCrLf
	if SESSION("clientID")="" AND noremember<>true then %>
if(frm.remember.checked==false&&!shipaddcan){
	if(confirm("<%=jscheck(xxWntRem)%>")){
...
if(!hasselected){
	alert("<%=jscheck(xxPlsEntr&" """&xxPlsChz)%>\".");
	return(false);
}
<%	elseif nodefaultpayprovider then %>
if(frm.payprovider.selectedIndex==0) return(chkfocus(frm.payprovider,"<%=jscheck(xxPlsEntr&" """&xxPlsChz)%>\"."));
<%	end if %>
return(!shipaddcan);
}
<% if termsandconditions then %>
...
elseif checkoutmode="authorize" then ' }{
	...
	if is_numeric(ordID) AND is_numeric(getpost("method")) then
		if getpayprovdetails(getpost("method"),data1,data2,data3,demomode,ppmethod) then
	...
	elseif getpost("method")="7" OR getpost("method")="8" OR getpost("method")="22" then ' Payflow Pro / Payflow Link / PayPal Advanced
		...
		if getpost("method")="8" OR getpost("method")="22" then
			...
			sXML=sXML & "&INVNUM=" & ordID & "&RETURNURL="&storeurlssl&"thanks.asp&ERRORURL="&storeurlssl&"thanks.asp&NOTIFYURL="&storeurlssl&"vsadmin/ppconfirm.asp&CREATESECURETOKEN=Y&TEMPLATE=MINLAYOUT&DISABLERECEIPT=TRUE&SECURETOKENID=" & securetokenid
			...
			if RESPMSG="Approved" then
				iframe="<iframe style=""border:none"" src=""https://payflowlink.paypal.com/?SECURETOKEN=" & SECURETOKEN & "&SECURETOKENID=" & securetokenid & IIfVs(demomode,"&MODE=test") & """ width=""510"" height=""610""></iframe>"
				print iframe
			else
				vsRESPMSG=RESPMSG
			end if
...
<%		else
	...
	elseif getpost("method")="18" then ' PayPal Direct
		...
		if vsAUTHCODE="" then
			if blockuser then
				...
				success=callxmlfunction("https://" & IIfVs(demomode,"SHA2-test-") & "api" & IIfVs(data2hash<>"","-3t") & sandbox & ".paypal.com/2.0/", sXML, res, IIfVr(data2hash<>"","",data1), "WinHTTP.WinHTTPRequest.5.1", vsRESPMSG, TRUE)
			end if
			if success then
	...
	if centinelenrolled<>"Y" AND ((getpost("method")<>"8" AND getpost("method")<>"22") OR iframe="") then
		call logevent(REMOTE_ADDR,"TRANSACTION",vsRESULT="0","cart.asp","ORDERS")
...
elseif checkoutmode="mailinglistsignup" then ' }{
	...
	if getpost("rp")<>"" then thehref=htmlspecials(replace(replace(getpost("rp"),"""",""),"<","")) else thehref=xxHomeURL
...
if getget("emailconf")<>"" OR getget("unsubscribe")<>"" then
	...
	print "</div><div class=""cartemailconfcontinue""><a class=""ectlink"" href=""" & xxHomeURL & """ onmouseover=""window.status='" & replace(xxCntShp,"'","\'") & "';return true"" onmouseout=""window.status='';return true"">" & xxCntShp & "</a></div>"
	print "</div>"
elseif getget("mode")="gw" then
	print "<form method=""post"" action=""cart.asp?mode=gw"">" & whv("doupdate","1") & "<div class=""cartgiftwrapdiv"">"
	...
	if getpost("doupdate")="1" then
		...
		print "<meta http-equiv=""Refresh"" content=""2; URL=cart.asp"">"
		print "<div class=""giftwrapupdating"">" & xxGifUpd & "</div>"
		print "<div class=""giftwrappleasewait"">" & xxPlsWait & " <a class=""ectlink"" href=""cart.asp"">" & xxClkHere & "</a>.</div></div>"
	else
		...
		if rs.EOF then
			...
			do while NOT rs.EOF
				print "<div class=""giftwrapline"">"
					...
					print "<div class=""giftwrapmessage""><div class=""giftwraptmessage"">" & xxGifMes & "</div><div class=""giftwraplinemessage""><textarea class=""gwmessage" & rs("cartID") & """ name=""gwmessage" & rs("cartID") & """ id=""gwmessage" & rs("cartID") & """ rows=""3"" cols=""34""" & IIfVr(rs("cartGiftWrap")=0, " disabled=""disabled""", "") & ">" & htmlspecials(rs("cartGiftMessage")) & "</textarea></div></div>"
				print "</div>"
				rs.movenext
			loop
			print "<div class=""giftwrapbuttons""><input type=""submit"" value=""Update Selections"" class=""giftwrapsubmit"" /> <input type=""button"" value=""" & xxCancel & """ onclick=""document.location='cart.asp'"" class=""giftwrapcancel"" /></div>"
		end if
		rs.close
	end if
	print "</div></form>"
elseif (left(getget("token"),2)<>"EC" OR checkoutmode="paypalcancel") AND (checkoutmode="dologin" OR checkoutmode="donewaccount" OR checkoutmode="update" OR checkoutmode="paypalcancel" OR checkoutmode="savecart" OR checkoutmode="movetocart" OR checkoutmode="") AND cartisincluded<>TRUE then ' {
	...
	sub writeestimatormenu()
		...
		elseif adminAltRates=1 then
			...
			if NOT rs.EOF then
				...
				do while NOT rs.EOF
					call writealtshipline(rs(getlangid("altratetext",65536)),rs("altrateid"),xxOrCom&": ",xxShEsWi&": ")
					rs.movenext
				loop
	...
	sub displaydiscounts()
		...
		if checkoutmode<>"savedcart" AND vsround(loyaltypointsavailable*loyaltypointvalue,2)>0 then
			...
			print "<select size=""1"" onchange=""document.location='cart.asp?redeempoints='+(this.selectedIndex==1?'no':'yes')""><option value="""">"&xxReLPts&"</option><option value=""""" & IIfVs(SESSION("noredeempoints")=TRUE," selected=""selected""") & ">" & xxSaLPts & " (" & loyaltypointsavailable & ")</option></select>"
	...
	if getget("mode")="logout" then
		...
		xxSryEmp=xxLOSuc
		call ectsetcookie("WRITECLL","x",365)
		call ectsetcookie("WRITECLP","",365)
		if storeurlssl<>storeurl then print "<script src=""" & storeurlssl & "vsadmin/savecookie.asp?WRITECLL=x&WRITECLP=&permanent=Y""></script>"
	end if
	addextrarows=0
	...
	if estimateshipping=TRUE then
		...
		if NOT nodiscounts AND NOT wantstateselector then
			sSQL="SELECT cpnID FROM coupons WHERE cpnCntry=1 AND cpnType=0 AND cpnNumAvail>0 AND cpnEndDate>=" & vsusdate(DateAdd("h",dateadjust,Now()))&" AND (cpnIsCoupon=0 OR (cpnIsCoupon=1 AND cpnNumber='"&rgcpncode&"')) AND ((cpnLoginLevel>=0 AND cpnLoginLevel<="&minloglevel&") OR (cpnLoginLevel<0 AND -1-cpnLoginLevel="&minloglevel&"))"
			rs.open sSQL,cnn,0,1
		...
		if wantstateselector then
			stateSQL="SELECT stateAbbrev,stateName,stateName2,stateName3,stateCountryID,countryName FROM states INNER JOIN countries ON states.stateCountryID=countries.countryID WHERE countryEnabled<>0 AND stateEnabled<>0 AND (stateCountryID=" & origCountryID & IIfVs((shipType=4 AND upsnegdrates=TRUE) OR origCountryID=1 OR origCountryID=2," OR stateCountryID=1 OR stateCountryID=2") & ") ORDER BY stateCountryID," & getlangid("stateName",1048576)
...
ectinwhichlist=inlist;
if(<%	if wantzipselector then print "document.getElementById('zip').value!=''&&"
		if wantstateselector then print "(document.getElementById('state').disabled||document.getElementById('state').selectedIndex!=0)&&"
%>true)return doupdate(document.forms.checkoutform);
}
...
function getalternatecarriers(){
	if(extraship.length>0){
		...
		ajaxobj=window.XMLHttpRequest?new XMLHttpRequest():new ActiveXObject("MSXML2.XMLHTTP");
		ajaxobj.onreadystatechange=acajaxcallback;
		ajaxobj.open("GET", "vsadmin/shipservice.asp?ratetype=estimator&best="+bestestimate+"&shiptype="+shiptype+"&sessionid=<%=urlencode(thesessionid)%>&destzip=<%=urlencode(destZip)%>&sc=<%=urlencode(shipcountry)%>&scc=<%=urlencode(shipCountryCode)%>&sta=<%=urlencode(shipStateAbbrev)%>", true);
...
document.getElementById('cartlinetot'+cartlineunique).innerHTML='<input type="button" class="cartlineupdate" value="<%=jscheck(xxUpdate)%>" onclick="actionupdate('+cid+',this.form,\''+listid+'\')" />';
}
/* ]]> */</script>
<%	cartliststring=""
	...
	print "<div class=""cartlists"">" ' {
	if (enableclientlogin=TRUE OR forceclientlogin=TRUE) AND NOT noclientloginprompt AND NOT amazonpaycheckout then
		print "<div class=""cartlistdiv""><div class=""ectdivhead cartlistname cartlistlogin"">"
		if SESSION("clientID")<>"" then
			print "<div class=""cartloggedin"">" & xxLogInA & " <span class=""cartloginname"">" & htmlspecials(SESSION("clientUser")) & "</span>"
			if enablewishlists then print " - <input type=""button"" onclick=""document.location='" & customeraccounturl&"#list';return false"" value="""&xxCreaGR&""" />"
			print " - " & imageorbutton(imgcartlogout,xxLogout,"ectlink","return dologoutaccount()",TRUE)
		...
		elseif noclientloginprompt<>TRUE then
			print "<div class=""loginprompt""><div class=""logintoaccount"">" & imageorbutton(imgloginaccount,xxLogAcc,"logintoaccount","displayloginaccount()",TRUE)&"</div>"
	...
	for cartlistnumber=0 to UBOUND(cartliststringarray)
		totalgoods=0 : totalquantity=0 : shipping=0 : stateTax=0 : countryTax=0 : totaldiscounts=0 : freeshipamnt=0 : loyaltypointdiscount=0
		handling=savehandling
		ispubliclist=FALSE
		print "<div class=""cartlistdiv"">" ' {
		...
		if enableclientlogin=TRUE OR forceclientlogin=TRUE then call displaysoftlogin()
		print "<form method=""post""" & IIfVs(checkoutmode=""," name=""checkoutform""") & " action=""" & cartpath & """" & IIfVs(isarray(alldata)," onsubmit=""return changechecker(this)""") & ">"
		...
		elseif IsArray(alldata) then
			if NOT amazonpaycheckout then
				if (stockwarning OR backorder) AND checkoutmode<>"savedcart" then
					...
					if stockwarning then
						print "<div class=""cartstockwarning""><div class=""cartoutstock ectwarning"">" & xxNoStok & "</div><div class=""cartstockacceptlevel"">"&xxStkUTo&"<a class=""ectlink"" href=""cart.asp"">"&xxClkHere&"</a></div>"
				...
				for index=0 to UBOUND(alldata,2)
					...
					if isnull(alldata(22,index)) AND alldata(1,index)<>donationid AND alldata(1,index)<>giftcertificateid then
				...
				print "<div class=""cartdetails cartdetailsname"">" & xxCOName & "</div>"
				...
				print "<div class=""cartdetails cartdetailscheck"">&nbsp;</div>"
			...
			for index=0 to UBOUND(alldata,2)
				...
				pDescription=trim(alldata(23,index)&"")
				pLongDescription=trim(alldata(24,index)&"")
				...
				do while NOT rs.EOF
					...
					theoptions=theoptions & "<div class=""cartoptionsline""><div class=""cartoptiongroup" & IIfVs(itemdeleted," ectwarning") & """>" & rs("coOptGroup") & "</div>" & _
						"<div class=""cartoption" & IIfVs(itemdeleted," ectwarning") & """>" & replace(replace(htmlspecials(rs("coCartOption")&"")," ","&nbsp;"),vbLf,"<br />") & "</div>"
				...
				if pDisplay<>0 AND linkcartproducts=TRUE AND (forcedetailslink OR pLongDescription<>"" OR pLargeImage<>"") then
					thedetailslink=getdetailsurl(cartProdID,pStaticPage,cartProdName,trim(pStaticURL&""),"",pathtohere)
					if detailslink<>"" then
				...
				if NOT amazonpaycheckout then ' Cart line
					print "<div class=""cartandoptsline""><div class=""cartline"">"
						...
						print "</div><div class=""cartlinename" & IIfVs(itemdeleted," ectwarning") & """>"
						print startlink & cartProdName & endlink
						if pGiftWrap<>0 then print "<div class=""giftwrap""><a href=""cart.asp?mode=gw"">" & IIfVr(cartGiftWrap<>0,xxGWrSel,xxGWrAva) & "</a></div>"
			...
			if NOT amazonpaycheckout AND NOT nopriceanywhere then
				...
				if estimateshipping=TRUE then
					...
					print IIfVs(xxEstimatorTitle<>"","<div class=""estimatortitle"">"&xxEstimatorTitle&"</div>") & "<div class=""estimatorerrors"" id=""estimatorerrors""><div class=""estimatorerror ectwarning"">" & errormsg & "</div></div>"
					...
					if wantstateselector then
						...
						print labeltxt(xxAllSta,"state")
						print "</div><div class=""cartstateselector" & IIfVs(invalidstate," ectwarning") & """>"
						print "<select name=""state"" id=""state"" class=""cartstateselector" & IIfVs(invalidstate," ectwarning") & """ size=""1"" onchange=""document.getElementById('updateestimator').style.display=''"">" : show_states(shipstate) : print "</select>"
						print "</div></div>" & vbCrLf
					end if
					if wantcountryselector then
						...
						print "<select name=""country"" id=""country"" size=""1"""&IIfVs(mobilebrowser," style=""font-size:10px""")&" onchange="""&IIfVs(wantstateselector,"dynamiccountries(this,'');")&"updateestimator('"&cartlistnumber&"')"">"
					...
					if wantzipselector then
						...
						print labeltxt(xxZip,"zip")
						print "</div><div class=""cartzipselector" & IIfVs(invalidzip," ectwarning") & """>"
						print "<input type=""text"" name=""zip"" id=""zip"" class=""cartzipselector" & IIfVs(invalidzip," ectwarning") & """ size=""8"" value=""" & htmlspecials(destZip) & """ onkeydown=""document.getElementById('updateestimator').style.display=''"" autocapitalize=""characters"" />"
						print "</div></div>" & vbCrLf
					end if
					if wantstateselector OR wantcountryselector OR wantzipselector then
						print "<div id=""updateestimator"" class=""updateestimator"" style=""display:none"">" & imageorbutton(imgupdateestimator,xxUpdEst,"updateestimator","updateestimator('"&cartlistnumber&"')",TRUE) & "</div>"
					end if
				end if
...
function amazonpaynow(){
	if(amznorderreferenceid!='')
		document.location='cart.asp?amzrefid='+amznorderreferenceid;
	else
...
<%					end if
				...
				elseif forceclientlogin=TRUE AND SESSION("clientID")="" then
					...
					do while NOT rs.EOF
						if rs("payProvID")=21 then
							if getget("access_token")<>"" AND getget("token_type")="bearer" AND getget("expires_in")<>"" AND getget("scope")<>"" then
								...
								if trim(SESSION("AmazonLogin"))<>"" AND trim(SESSION("AmazonLoginTimeout"))<>"" then response.redirect storeurlssl & "cart.asp?amazonpay=go"
							end if
						end if
				...
				print "<div class=""checkoutopts checkoutbutton1 coopt" & rs("payProvID") & """>" & imageorsubmit(imgcheckoutbutton,xxCOTxt&""" onclick=""document.forms.checkoutform.action='"&cartpath&"';document.forms.checkoutform.cart.value='';document.forms.checkoutform.mode.value='checkout';"" title="""&xxPrsChk,"checkoutbutton checkoutbutton1") & "</div>" & vbCrLf
						end if
						rs.movenext
					loop
					rs.close
...
<%			session.LCID=saveLCID
			...
			if checkoutmode="savedcart" then
				...
				print "<div class=""cartemptyclickhere"">" & IIfVr(getget("pli")<>"",xxEmpWis,xxGetSta & " <a class=""ectlink"" href="""&xxHomeURL&"""><strong>"&xxClkHere&"</strong></a>.") & "</div>"
			end if %>
<script type="text/javascript">/* <![CDATA[ */
...
<%			print "<div class=""emptycartcontinue"">"
			if thefrompage<>"" AND (actionaftercart=2 OR actionaftercart=3) then thehref=htmlspecials(thefrompage) else thehref=xxHomeURL
