...
function checkcustomerblock()
	if NOT isectadmin AND sDSN<>"" then
...
<%		end if
%><input type="hidden" id="hiddencurr" value="<%=replace(FormatEuroCurrency(0),"""","&quot;")%>"><div id="opaquediv" class="ectopaque" style="display:none" onclick="if(event.target==this)this.style.display='none'"></div><%
...
response.write "<input type=""hidden"" name=""" & hvname & """ value=""" & htmlspecials(hvval) & """>" & vbLf
end sub
function whv(hvname,hvval)
whv="<input type=""hidden"" name=""" & hvname & """ value=""" & htmlspecials(hvval) & """>" & vbLf
end function
sub print(ps)
response.write ps
end sub
sub writehiddenidvar(hvname,hvval)
response.write "<input type=""hidden"" name=""" & hvname & """ id=""" & hvname & """ value=""" & htmlspecials(hvval) & """>" & vbLf
end sub
...
function displayproductoptions(byRef optpricediff,thetax,isdetail, byRef hasmulti, byRef optionsjs)
	...
	do while rowcounter<=maxindex
		...
		if NOT rs2.EOF then
			...
			optplaceholder=trim(rs2(getlangid("optPlaceholder",16))&"")
			if opttooltip<>"" then
				if left(opttooltip,2)="##" then
					opttooltip=right(opttooltip,len(opttooltip)-2)
				else
					opttooltip="<div class=""opttooltip"" "&IIfVr(mobilebrowser,"onclick=""thisstyle=this.getElementsByTagName('div')[0].style;thisstyle.display=thisstyle.display=='inline'?'none':'inline'""","onmouseover=""this.getElementsByTagName('div')[0].style.display='inline'"" onmouseout=""this.getElementsByTagName('div')[0].style.display='none'""")&"><div class=""tooltipcontent" & IIfVs(optionnameaspleaseselect AND abs(int(rs2("optType")))<>1," tooltipright") & """>"&opttooltip&"</div></div>"
				end if
			end if
			if abs(int(rs2("optType")))=5 then ' Date Picker
				if optionnameaspleaseselect AND optplaceholder="" then optplaceholder=rs2(getlangid("optGrpName",16))
				opthasstock=TRUE
				...
				optshtml=optshtml&"<div class=""optioncontainer " & IIfVr(isdetail,"detail","prod") & "optioncontainer ectdpoption" & IIfVs(optclass<>""," "&optclass) & """"&IIfVs(prodoptions(5,rowcounter)=1," style=""display:none"" id=""divc"&Count&"x"&rowcounter&"""")&">"
				if NOT optionnameaspleaseselect then optshtml=optshtml&"<div class=""optiontext" & IIfVs(isdetail," detailoptiontext") & """><label for=""optn"&Count&"x"&rowcounter&""">"&rs2(getlangid("optGrpName",16))&"</label>"&opttooltip&"</div>"
				optshtml=optshtml&"<div class=""option" & IIfVs(isdetail," detailoption") & """> <input data-optgroup="""&prodoptions(0,rowcounter)&""" "&IIfVs(prodoptions(5,rowcounter)=1,"data-isdep=""1"" ")&"type=""hidden"" name=""optn"&rowcounter&""" value="""&rs2("optID")&""">"
				optshtml=optshtml&"<div class=""popupcaldiv"" style=""position:relative;display:inline""><input onclick=""ectremoveclass(this,'ectwarning');popUpCalendar(this,this,'"&themask&"')"" data-optgroup="""&prodoptions(0,rowcounter)&""" "&IIfVs(prodoptions(5,rowcounter)=1,"data-isdep=""1"" ")&"type=""text"" class=""ecttextinput prodoption"&IIfVr(isdetail," detailprodoption","") & """ maxlength=""255"" name=""voptn"&rowcounter&""" id=""optn"&Count&"x"&rowcounter&""" size="""&int(rs2("optDims"))&""" value=""" & htmldisplay(rs2(getlangid("optName",32))) & """" & IIfVs(optplaceholder<>""," placeholder=""" & optplaceholder & """") & IIfVs(prodoptions(5,rowcounter)=1," disabled=""disabled""")&"></div>"
				optshtml=optshtml&IIfVs(optionnameaspleaseselect,opttooltip)&"</div></div>"
			elseif abs(int(rs2("optType")))=3 then ' Text
				if optionnameaspleaseselect AND optplaceholder="" then optplaceholder=rs2(getlangid("optGrpName",16))
				opthasstock=TRUE
				optclass=trim(rs2("optClass")&"")
				fieldHeight=cint((cdbl(rs2("optDims"))-int(rs2("optDims")))*100.0)
				optshtml=optshtml&"<div class=""optioncontainer " & IIfVr(isdetail,"detail","prod") & "optioncontainer ecttextoption" & IIfVs(optclass<>""," "&optclass) & """"&IIfVs(prodoptions(5,rowcounter)=1," style=""display:none"" id=""divc"&Count&"x"&rowcounter&"""")&">"
				if NOT optionnameaspleaseselect then optshtml=optshtml&"<div class=""optiontext" & IIfVs(isdetail," detailoptiontext") & """><label for=""optn"&Count&"x"&rowcounter&""">"&rs2(getlangid("optGrpName",16))&"</label>"&opttooltip&"</div>"
				optshtml=optshtml&"<div class=""option" & IIfVs(isdetail," detailoption") & """> <input data-optgroup="""&prodoptions(0,rowcounter)&""" "&IIfVs(prodoptions(5,rowcounter)=1,"data-isdep=""1"" ")&"type=""hidden"" name=""optn"&rowcounter&""" value="""&rs2("optID")&""">"
				if fieldHeight<>1 then
					optshtml=optshtml&"<textarea onkeydown=""ectremoveclass(this,'ectwarning')"" data-optgroup="""&prodoptions(0,rowcounter)&""" "&IIfVs(prodoptions(5,rowcounter)=1,"data-isdep=""1"" ")&"class=""ecttextinput prodoption"&IIfVr(isdetail," detailprodoption","")&""" name=""voptn"&rowcounter&""" id=""optn"&Count&"x"&rowcounter&""" cols="""&int(rs2("optDims"))&""" rows="""&fieldHeight&"""" & IIfVs(optplaceholder<>""," placeholder=""" & optplaceholder & """") & IIfVs(prodoptions(5,rowcounter)=1," disabled=""disabled""") & ">"
					optshtml=optshtml&rs2(getlangid("optName",32))&"</textarea>"
				else
					optshtml=optshtml&"<input onkeydown=""ectremoveclass(this,'ectwarning')"" data-optgroup="""&prodoptions(0,rowcounter)&""" "&IIfVs(prodoptions(5,rowcounter)=1,"data-isdep=""1"" ")&"type=""text"" class=""ecttextinput prodoption"&IIfVr(isdetail," detailprodoption","")&""" maxlength=""255"" name=""voptn"&rowcounter&""" id=""optn"&Count&"x"&rowcounter&""" size="""&int(rs2("optDims"))&""" value=""" & htmldisplay(rs2(getlangid("optName",32))) & """" & IIfVs(optplaceholder<>""," placeholder=""" & optplaceholder & """") & IIfVs(prodoptions(5,rowcounter)=1," disabled=""disabled""")&">"
				end if
				optshtml=optshtml&IIfVs(optionnameaspleaseselect,opttooltip)&"</div></div>"
			elseif abs(int(rs2("optType")))=1 then ' Checkbox / Radio
				...
				if NOT noradiooptionlabel then optshtml=optshtml&"<div class=""optiontext" & IIfVs(isdetail," detailoptiontext") & """>"&rs2(getlangid("optGrpName",16))&opttooltip&"</div>"
				...
				do while not rs2.EOF
					...
					origoptpricediff=getoptpricediff(rs2("optPriceDiff"), rs("pID"), trim(rs2("optRegExp")&""), rs("pPrice"), stocknotused)
					call addtoaltids(rs2("optRegExp"), altidarr, altids)
					optshtml=optshtml&"<div class=""rcoption" & IIfVs((rs2("optFlags") AND 4)=4," rcoptioninline") & IIfVs(optclass<>""," "&optclass) & """><input type="""&IIfVr(rs2.recordcount=1,"checkbox","radio")&""" data-optgroup="""&prodoptions(0,rowcounter)&""" "&IIfVs(isdependent,"disabled=""disabled"" data-isdep=""1"" ")&"class=""ect"&IIfVr(rs2.recordcount=1,"checkbox","radio")&" prodoption"&IIfVs(isdetail," detailprodoption")&""" onclick=""updateoptimage("&Count&","&rowcounter&",1)"" name=""optn"&Count&"x"&rowcounter&""" "
			...
			elseif abs(int(rs2("optType")))=4 then ' Multi
				...
				if cint(rs2("optGrpSelect"))<>0 AND NOT isdetail then
					...
					do while not rs2.EOF
						...
						optshtml=optshtml&"&nbsp;&nbsp;<input data-optgroup="""&prodoptions(0,rowcounter)&""" "&IIfVs(prodoptions(5,rowcounter)=1,"data-isdep=""1"" ")&"type=""text"" maxlength=""5"" name=""optm"&rs2("optID")&""" id=""optm"&Count&"x"&rowcounter&"y"&index&""" size=""1"" "&IIfVr(oostock,"style=""background-color:#EBEBE4"" disabled=""disabled""","")&" class=""ecttextinput"">"
				...
				if NOT noselectoptionlabel AND NOT optionnameaspleaseselect then optshtml=optshtml&"<div class=""optiontext" & IIfVs(isdetail," detailoptiontext") & """><label for=""optn"&Count&"x"&rowcounter&""">"&rs2(getlangid("optGrpName",16))&"</label>"&opttooltip&"</div>"
				optshtml=optshtml&"<div class=""option" & IIfVs(isdetail," detailoption") & """><select data-optgroup="""&prodoptions(0,rowcounter)&""" "&IIfVs(prodoptions(5,rowcounter)=1,"data-isdep=""1"" ")&"class=""prodoption"&IIfVr(isdetail," detailprodoption","")&""" onchange=""updateoptimage("&Count&","&rowcounter&",2)"" name=""optn"&rowcounter&""" id=""optn"&Count&"x"&rowcounter&""" "&IIfVs(prodoptions(5,rowcounter)=1,"disabled=""disabled"" ")&"size=""1"">"
				defjs=defjs & "document.getElementById('optn"&Count&"x"&rowcounter&"').onchange();"
				gotdefaultdiff=FALSE
				firstpricediff=0
				origoptpricediff=rs2("optPriceDiff")
				if cint(rs2("optGrpSelect"))<>0 then
					if optionpleaseselecttemplate="" then optionpleaseselecttemplate="Please Select: %s..."
...
sub displayformvalidator()
	optjs=optjs&"function formvalidator"&Count&"(theForm){" & vbLf & _
					"var fvsuccess=true;" & vbLf
	if isarray(prodoptions) then
		optjs=optjs&"setvals"&Count&"();"
		optjs=optjs&"fvsuccess=ectvalidate(theForm,"&(UBOUND(prodoptions,2)+1)&","&Count&","&IIfVr(useStockManagement AND cint(rs("pStockByOpts"))<>0, "true", "false")&","&IIfVr(cint(rs("pBackOrder"))<>0, "true", "false")&")"&IIfVr(customvalidator<>"","&&customvalidator(theForm)","")&";"
	end if
	if googletagid<>"" then
		optjs=optjs&"if(fvsuccess) gtag(""event"",""add_to_cart"",{ currency:'" & countryCurrency & "',value:" & FormatNumber(totprice,2,-1,0,0) & ",items:[{item_id:'" & jsescapel(rs("pId")) & "'," & _
			"item_name:'" & jsescapel(rs(getlangid("pName",1))) & "',index:" & Count & ","
		if wantmanufacturer then
			if trim(rs(getlangid("scName",131072)))<>"" then optjs=optjs&"item_brand:'" & jsescapel(rs(getlangid("scName",131072))) & "',"
		end if
		if catname<>"" then optjs=optjs&"item_category:'" & jsescapel(catname) & "',"
		optjs=optjs&"price:" & FormatNumber(totprice,2,-1,0,0) & ",quantity:1}]});"&vbLf
	end if
	optjs=optjs&"return("&IIfVr(customvalidator<>"","customvalidator(theForm)","fvsuccess")&");}"&vbLf
end sub
...
function imageorlink(theimg,thetext,theclass,thelink,isjs)
	...
	elseif theimg<>"" then
		...
		imageorlink="<a class=""ectlink"&IIfVs(theclass<>""," "&theclass)&""" href="""&IIfVs(isjs,"#"" onclick=""")&thelink&""">"&thetext&"</a>"
	end if
end function
function imageorbuttontag(theimg,thetext,theclass,thelink,isjs)
	isabsolute=instr(thelink,"http://")>0 OR instr(thelink,"https://")>0
	if theimg="link" then
		imageorbuttontag="<a class=""ectlink"&IIfVs(theclass<>""," "&theclass)&""" href="""&IIfVs(isjs,"#"" onclick=""return ")&thelink&""">"&thetext&"</a>"
...
function imageorbutton(theimg,thetext,theclass,thelink,isjs)
	isabsolute=instr(thelink,"http://")>0 OR instr(thelink,"https://")>0
	if theimg="link" then
		imageorbutton="<a class=""ectlink"&IIfVs(theclass<>""," "&theclass)&""" href="""&IIfVs(isjs,"#"" onclick=""return ")&thelink&""">"&thetext&"</a>"
	elseif theimg<>"" AND theimg<>"button" then
...
function showproductreviews(disptype, classname)
	...
	next
	showproductreviews=showproductreviews & "</a><div class=""" & classname & "text"">"
	...
	showproductreviews=showproductreviews & "</div></div>"
end function
...
quantitymarkup="<div class="""&placeholder&"quantity0div" & IIfVr(isdetail," detail"," prod") & "quantity0div""><input type=""text"""&IIfVs(NOT isquickbuy," name=""quant""")&" id=""w"&IIfVs(isquickbuy,"qb")&Count&"quant"" size=""2"" maxlength=""5"" value=""1"" title="""&xxQuant&""" " & IIfVs(addonchange,"onchange=""document.getElementById('qnt"&Count&"x').value=this.value"" ") & "class=""ecttextinput quantity0input" & IIfVr(isdetail," detail"," prod") & "quantity0input""></div>"&vbLf
end if
end function
...
sub displaylayoutarray(layoutarray,isquickbuy)
	...
	for each layoutoption in layoutarray
		...
		elseif layoutoption="productname" then
			...
			print "<div class="""&cs&"prodname"">"&startlink & rs(getlangid("pName",1)) & endlink & xxDot & "</div>" & vbLf
		...
		elseif layoutoption="addtocartquant" then
			print "<div class=""addtocartquant prodaddtocartquant""><div class=""prodquantity"">" & quantitymarkup(isquickbuy,Count,FALSE,"",FALSE) & "</div>" & replace(IIfVr(isquickbuy,atcmuqb,atcmu),"XXXECTCSPLACEHOLDERXXXaddtocart",cs&"addtocart") & "</div>"
		...
		elseif layoutoption="quickbuy" then
			print "<div class=""qbopaque"" id=""qbopaque"&Count&""" style=""display:none"" onclick=""if(event.target==this)closequickbuy("&Count&")"">"
...
<%		print "<div id=""alopaquediv"" class=""ectopaque"" style=""display:none"" onclick=""if(event.target==this)hideaccounts()"">"
		...
		print "<div class=""cartlogincookie_cntnr flexvertalign""><div class=""cartlogincookie""><input type=""checkbox"" class=""ectcheckbox"" id=""licook""></div><div class=""cartlogincookietext"">" & labeltxt(xxRemLog,"licook") & "</div></div>"
		...
		print "</div>"
		' *****
		print "<div id=""acopaquediv"" class=""ectopaque"" style=""display:none"" onclick=""if(event.target==this)hideaccounts()"">"
		...
		print "<div class=""cartnewaccount_cntnr"" id=""newacctdiv"">"
			...
			if nomailinglist<>TRUE then
				...
				print "<div class=""cartaccloginallowpromo""><input type=""checkbox"" class=""ectcheckbox"" name=""allowemail"" id=""allowemail"" value=""ON""" & IIfVs(allowemaildefaulton=TRUE OR getpost("allowemail")="ON"," checked=""checked""") & "></div>"
...
sub addtogoogleanalyticsplist(dbrs,ao,index)
	if ao<>"" then ao=ao & ","
	ao=ao & "{item_id:'" & jsescapel(dbrs("pId")) & "'," & _
		"item_name:'" & jsescapel(dbrs(getlangid("pName",1))) & "'," & _
		"index:" & index & ","
	if wantmanufacturer then
		if trim(dbrs(getlangid("scName",131072))&"")<>"" then ao=ao & "item_brand:'" & jsescapel(dbrs(getlangid("scName",131072))) & "',"
	end if
	if catname<>"" then ao=ao & "item_category:'" & jsescapel(catname) & "',"
		ao=ao & "price:" & jsescapel(totprice) & "}"
end sub
