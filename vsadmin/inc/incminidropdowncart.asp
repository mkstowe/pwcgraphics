...
if request.form("mode")="checkout" then
	if trim(request.form("checktmplogin"))<>"" AND is_numeric(request.form("checktmplogin")) then
...
do while NOT rs2.EOF
	...
	if productimageurl="" then
		...
		mcpdtxt=mcpdtxt&"<img src="""&productimageurl&""" class=""ectdp_minicartImg"" width=""40px"" height=""40px"">"
	end if
	mcpdtxt=mcpdtxt&"</div><div class=""ectdp_minicartitemName"">" & IIfVs(nopriceanywhere,productqty&" ") & rs2("cartProdName") & IIfVs(NOT nopriceanywhere,"<br /><span class=""ectMCtot"">" & productqty & " x " & FormatEuroCurrency(productprice) & "</span>") & "</div><div class=""ectdp_minicartitemDelete""><a href=""javascript:dodeleteitem(" & rs2("cartID") & ")"" class=""deletecartitem"">X</a></div><div style=""clear:both;""></div></div>"
	rs2.MoveNext
loop
rs2.close
