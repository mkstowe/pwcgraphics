...
if SESSION("loginid")=0 AND getget("act")="events" then
...
count=1
prevbought=0
sSQL="SELECT "&IIfVs(mysqlserver<>TRUE,"TOP 10 ")&"cartProdName, SUM(cartQuantity) AS numbought, SUM(cartProdPrice*cartQuantity) AS totalvalue FROM cart INNER JOIN orders ON cart.cartOrderID=orders.ordID WHERE "
if tsdays>0 then sSQL=sSQL&"cartDateAdded>=" & vsusdate(dbdate)&" AND "
sSQL=sSQL&"cartCompleted=1 AND ordStatus>=3 GROUP BY cartProdName ORDER BY SUM(cartQuantity) DESC, SUM(cartProdPrice*cartQuantity) DESC"&IIfVs(mysqlserver=TRUE," LIMIT 0,10")
rs.open sSQL,cnn,0,1
do while NOT rs.EOF
	print "<tr><td style=""width:10%""><strong>"&count&"</strong></td><td style=""text-align:left;"">"&rs("cartProdName")&"</td><td>"&rs("numbought")&"</td><td style=""text-align:right;"">"&FormatEuroCurrency(rs("totalvalue"))&"</td></tr>"
	count=count+1
	rs.movenext
loop
rs.close
