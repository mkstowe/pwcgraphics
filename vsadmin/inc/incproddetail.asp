...
catname="" : analyticsoutput="" : WSP="" : OWSP=""
TWSP="pPrice"
Count=0
...
nopreviousnextlinks=instr(detailpagelayout,"previousnext")=0
...
function showreviews(theSQL,showall)
	...
	if NOT rs2.EOF then
		...
		showreviews=showreviews & "<div class=""reviewscontainer"">"
		...
		showreviews=showreviews & "<div class=""noreview"">" & xxRvNone & "</div>"
	end if
	rs2.close
	if NOT (onlyclientratings AND SESSION("clientID")="") then showreviews=showreviews & "<div class=""clickreview"">" & imageorbuttontag(imgclicktoreview,xxClkRev,"clickreview",detailpageurl("review=true"&IIfVr(thecatid<>"","&amp;cat="&thecatid,"")), FALSE) & "</div>"
...
sub pddreviewstars(issmall)
	if rs("pNumRatings")>0 then
		...
		next
		print "</a>"
		if detailreviewstarstext<>"" then print "<div class=""detailratingtext"">" & replace(replace(replace(detailreviewstarstext,"%numratings%",rs("pNumRatings")),"%totrating%",vsround(rs("pTotRating")/rs("pNumRatings")/2,1)),"%reviewlink%",detailpageurl("")&"#reviews") & "</div>"
		print "</div>"
	else
...
<%	end if
	...
	/* ]]> */
		...
		<div class="reviewformblock">
			<div class="ectformline reviewformline"><div class="reviewlabels"><%=redstar & xxRvRati%>:</div><div class="reviewfields"><select size="1" name="ratingstars" id="ratingstars" class="ectselectinput reviewform"><option value=""><%=xxPlsSel%></option><%
				for index=1 to 5
					print "<option value="""&index&""">"&index&" "&xxStars&"</option>"
				next %></select></div></div>
			<div class="ectformline reviewformline"><div class="reviewlabels"><%=redstar & xxRvPosb%>:</div><div class="reviewfields"><input type="text" size="20" name="reviewposter" id="reviewposter" maxlength="64" value="<%=htmlspecials(SESSION("clientUser"))%>" class="ecttextinput reviewform" /></div></div>
			<div class="ectformline reviewformline"><div class="reviewlabels"><%=redstar & xxRvHead%>:</div><div class="reviewfields"><input type="text" size="40" name="reviewheading" maxlength="253" class="ecttextinput reviewform" /></div></div>
			<div class="ectformline reviewformline"><div class="reviewlabels"><% if reviewcommentsminlength<>"" then print redstar %><%=xxRvComm%>:</div><div class="reviewfields"><textarea name="reviewcomments" cols="38" rows="8" class="ecttextinput reviewform"></textarea></div></div>
...
sub pdddetailname()
	print "<div class=""detailname""><h1"&IIfVs(NOT noschemamarkup," itemprop=""name" & IIfVs(isarticle," headline") & """") & ">" & rs(getlangid("pName",1))&"</h1>"&xxDot
...
for each layoutoption in customlayoutarray
	...
	if lcase(trim(layoutoption))="manufacturer" OR lcase(trim(layoutoption))="manufacturerlink" then wantmanufacturer=TRUE
		...
		for index=0 to 10
			...
			elseif index=10 then
				...
				if NOT rs2.EOF then
					...
					if thetopts=catid then catname=rs2(getlangid("sectionName",256))
...
<div id="giantimgspan" class="ectopaque" style="display:none" onclick="if(event.target==this)hidegiantimage()">
...
<%				end if %>
		<div class="giantimg" style="margin:3px 10px 4px 4px;flex-grow:1">
			<div class="giantimgheader">
				...
				<div class="giantimgclose" style="float:right;position:absolute;top:20px;right:2%"><img src="images/close.gif" style="margin:5px 0;;cursor:pointer" alt="<%=xxClsWin%>" onclick="hidegiantimage()"></div>
			</div>
...
<%			end if
	...
	if isarray(prodoptions) AND request("review")="" then
		if usedetailbodyformat=1 OR usedetailbodyformat="" then
			optionshtml=displayproductoptions(optdiff,thetax,TRUE,hasmultipurchase,optjs)
		else
			optionshtml=displayproductoptions(optdiff,thetax,TRUE,hasmultipurchase,optjs)
		end if
	end if
...
<%		if showproductid=TRUE then print "<div class=""detailid""><strong>" & xxPrId & ":</strong> " & IIfVs(NOT noschemamarkup,"<span itemprop=""productID"">") & rs("pID") & IIfVs(NOT noschemamarkup,"</span>") & "</div>"
		...
		print "<div class=""detailname""><h1"&IIfVs(NOT noschemamarkup," itemprop=""name""") & ">" & rs(getlangid("pName",1))&"</h1>"&xxDot
		if alldiscounts<>"" then print " <span class=""discountsapply detaildiscountsapply"">"&xxDsApp&"</span></div>"&"<div class=""detaildiscounts"">" & alldiscounts & "</div>" else print "</div>"
...
<%		if nobuyorcheckout=TRUE then
			...
			elseif isinstock OR isbackorder then
				...
				if notifybackinstock then
					...
					print "<div class=""outofstock detailoutofstock"">" & xxOutStok & "</div>"
				end if
			end if
		end if %></td>
			  </tr>
...
<%	else ' }{ if usedetailbodyformat=2/3/4
	...
	if getget("review")<>"true" AND prodid<>giftcertificateid AND prodid<>donationid then print "</form>"
	if NOT usecsslayout then print "</td></tr>"
	if NOT hascustomlayout OR request("review")<>"" then call pddreviews()
	if usecsslayout then print "</div>" else print "</table>"
	if prodid<>giftcertificateid AND prodid<>donationid then call addtogoogleanalyticsplist(rs,analyticsoutput,Count+1)
end if ' } rs.EOF
rs.close
set rs=nothing
set rs2=nothing
...
if googletagid<>"" AND analyticsoutput<>"" then
	print "<script>gtag(""event"",""view_item"",{ currency:'" & countryCurrency & "',value:" & totprice & ",items:[" & analyticsoutput & "]});</script>"&vbLf
end if
