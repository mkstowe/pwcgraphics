			...
			<div class="searchcntnr searchfor_cntnr">
				...
				<div class="searchcontrol searchfor">
					<div style="position:relative">
						<input type="search" name="stext" id="stext" class="ecttextinput" size="20" maxlength="1024" value="<%=htmlspecials(request("stext"))%>" onkeydown="return ectAutoSrchKeydown(this,event,'asp')" onblur="ectAutoHideCombo(this)" <%=noautocorrect%> />
					</div>
...
<%	end if %>
			...
			<div class="searchcntnr searchprice_cntnr">
				...
				<div class="searchcontrol searchprice"><input type="number" name="sprice" class="ecttextinput" size="10" maxlength="64" value="<%=htmlspecials(request("sprice"))%>" <%=noautocorrect%> /></div>
			</div>
			<div class="searchcntnr searchtype_cntnr">
				<div class="searchtext searchtypetext"><%=xxSrchTp%></div>
				<div class="searchcontrol searchtype"><select class="ectselectinput searchtype" name="stype" size="1">
...
<%	end if
	if (prodfilter AND 8)=8 AND sortoptions<>0 AND NOT nosortsearch then %>
			<div class="searchcntnr searchsort_cntnr">
				...
				<div class="searchcontrol searchsort"><select name="sortby" class="ectselectinput searchsort" size="1">
...
%>			<div class="searchcntnr searchfilters_cntnr">
			...
			do while NOT rs2.EOF
				if currgroup<>rs2("scGroup") then
					...
					print "<div class=""searchfiltergroup"&rs2("scGroup")&" searchfiltergroup""><select class=""ectselectinput searchfiltergroup"" name=""sfil"&rs2("scGroup")&""" size=""1""><option style=""font-weight:bold"" value="""">== All " & htmlspecials(rs2("scgTitle")) & " ==</option>"
