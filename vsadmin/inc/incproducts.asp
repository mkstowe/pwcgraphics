			...
			if isarray(ectfiltercache) then
				for cacheindex=0 to UBOUND(ectfiltercache,2)
					if currgroupid<>ectfiltercache(3,cacheindex) then
						...
						searchcriterialist=searchcriterialist&"<select name=""scri"" class=""prodfilter"" id=""scri"&maxgroupid&""" onchange=""filterbyman(1)"""&IIfVr(sidefilterstyle="multiple"," size=""5"" multiple=""multiple"""," size=""1""")&"><option value="""" style=""font-weight:bold"">== All " & ectfiltercache(5,cacheindex) & " ==</option>" & vbCrLf
...
%>			  </select><%
			...
			if (prodfilter AND 32)=32 then
				...
				print "<div class=""prodfilter ectpfkeyword"">" %><input onkeydown="changelocfiltertext(event.keyCode,this)" type="text" class="prodfilter" size="20" id="ectfilter" name="filter" value="<%=htmlspecials(getget("filter"))%>" /><%
