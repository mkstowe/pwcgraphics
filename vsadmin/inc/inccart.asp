...
sub writealtshipline(altmethod,altid,pretext,defpretext,isestimator)
	if NOT isestimator then
		...
		if shippingoptionsasradios=TRUE then
			if altmethod<>"" OR origShipType=altid then print "<div class=""shipline""" & IIfVs(shipType=altid," style=""font-weight:bold""") & "><label class=""ectlabel shipradio""><input type=""radio"" class=""shipradio"" value="""&altid&""""&IIfVr(shipType=altid," checked=""checked""","")&" onclick=""selaltrate("&altid&")"" /><span class=""shiplinetext"">" & IIfVr(shipType=altid,defpretext,pretext) & altmethod & "</span></label></div>"
		else
...
sub writeshippingoption(soindex,shipcost,freeshipexempt,freeship,shipmethod,isselected,wsodelivery)
	...
	if shippingoptionsasradios=TRUE then
		print "<div class=""shiprateline""><label class=""ectlabel shipradio""><input type=""radio"" class=""shipradio"" value=""RATE"""&IIfVr(isselected, " checked=""checked""", "")&" onclick=""updateshiprate(this,"&numshiprate&")"" /><span class=""shipratemethod"&IIfVs(numshiprateingroup=0," shipratemethodselected")&""">"&shipmethod&" "&IIfVr(combineshippinghandling, FormatEuroCurrency((shipcost+wsohandling)-wsofreeshipamnt), FormatEuroCurrency(shipcost-wsofreeshipamnt))&"</span>"&IIfVs(wsodelivery<>"","<div class=""cart3servicecommitment"">(" & wsodelivery & ")</div>")&"</label></div>"
	else
...
sub writeshippingflags(costage)
	...
	if willpickuptext<>"" OR willpickup_ then %>
			<div class="billformrowflags"><div class="cdshipftflag cdformtwillpickup"><input type="checkbox" name="willpickup" id="shipflag0" value="Y" <%
...
<%	end if
	if NOT willpickup_ then
		if commercialloc=TRUE OR commercialloc=2 then %>
			<div class="billformrowflags"><div class="cdshipftflag"><input type="checkbox" name="commercialloc" id="shipflag1" value="Y" <%
...
<%		end if
		if saturdaydelivery=TRUE then %>
			<div class="billformrowflags"><div class="cdshipftflag"><input type="checkbox" name="saturdaydelivery" id="shipflag2" value="Y" <%
...
<%		end if
		if abs(addshippinginsurance)=2 then %>
			<div class="billformrowflags"><div class="cdshipftflag"><%
				if forceinsuranceselection AND costage<>3 then
					...
					print "<input type=""checkbox"" name=""wantinsurance"" id=""shipflag3"" value=""Y"" "&IIfVs((ordComLoc AND 2)=2,"checked=""checked"" ")&IIfVs(costage=3,"onchange=""setchangeflag(this.checked,1)"" ")&"/>"
				end if %></div>
...
<%		end if
		if allowsignaturerelease=TRUE AND signatureoption<>"" then %>
			<div class="billformrowflags"><div class="cdshipftflag"><input type="checkbox" name="signaturerelease" id="shipflag4" value="Y" <%
...
<%		end if
		if insidedelivery=TRUE then %>
			<div class="billformrowflags"><div class="cdshipftflag"><input type="checkbox" name="insidedelivery" id="shipflag5" value="Y" <%
...
<%		end if
		if holdatlocation=TRUE then %>
			<div class="billformrowflags"><div class="cdshipftflag"><input type="checkbox" name="holdatlocation" id="shipflag6" value="Y" /></div>
...
<%		end if
		if homedelivery=TRUE then %>
			...
			<div class="cdshipflag"><select name="homedelivery" size="1">
...
<%		end if
	...
	if termsandconditions AND costage=3 AND (ordPayProvider="19" OR ordPayProvider="21") then %>
		<div class="billformrowflags">
		  <div class="cdshiptterms"><input type="checkbox" name="termsandconds" id="ecttnccheckbox" value="1" onclick="document.getElementById('sftermsandconds').value=this.checked?1:''" <%=IIfVs(getpost("sftermsandconds")="1","checked=""checked"" ")%>/></div>
...
<%		end if
		if ordPayProvider="19" AND getget("token")<>"" then %>
			  <div class="cart3row">
				<div class="cobhl cobhl3"><%=xxAddInf%></div>
				<div class="cobll cobll3"><textarea name="ordAddInfo" class="addinfo"<% if addinfplaceholder<>"" then print " placeholder=""" & addinfplaceholder & """"%>><%=htmlspecials(ordAddInfo)%></textarea></div>
			  </div>
<%		end if
		if backorder then %>
			  <div class="cart3row">
...
<%			end if %>
			  ...
			  <div class="cart4row">
				...
				<div class="cobll cobll4"><input class="ecttextinput nofixw" type="text" name="<%=sscvv2%>" id="<%=sscvv2%>" size="4" AUTOCOMPLETE="off" /> <%if requirecvv<>TRUE then print xxIfPres%></div>
			  </div>
<%			if Mid(data1,8,1)="X" then %>
			  <div class="cart4row">
...
<%		sub writeaddressspans(isshp) %>
		...
		next %></select> <div class="editaddressbuttons"><input type="button" class="ectbutton" value="<%=xxEdit%>" id="<%=isshp%>editbutton" onclick="editaddress('<%=isshp%>',false);document.getElementById('<%=isshp%>addressspan1').style.display='none';document.getElementById('<%=isshp%>addressspan2').style.display='block';document.getElementById('<%=isshp%>addaddress').value='edit';"> <input type="button" class="ectbutton" value="<%=xxNew%>" onclick="editaddress('<%=isshp%>',true);document.getElementById('<%=isshp%>addressspan1').style.display='none';document.getElementById('<%=isshp%>addressspan2').style.display='block';document.getElementById('<%=isshp%>addaddress').value='add';" /></div>
			...
			if usefirstlastname then
				print "<input type=""text"" name="""&isshp&"ordname"" autocomplete=""given-name"" id="""&isshp&"ordname"" class=""cdformsmfixw"" placeholder="""&stripnspecials(xxFirNam)&""" /> <input type=""text"" name="""&isshp&"lastname"" autocomplete=""family-name"" id="""&isshp&"lastname"" class=""cdformsmfixw"" placeholder="""&stripnspecials(xxLasNam)&""" />"
			else
				print "<input type=""text"" name="""&isshp&"ordname"" id="""&isshp&"ordname"" class=""cdform1fixw"" placeholder="""&stripnspecials(xxName)&""" />"
			end if
			%></div></div>
			<div class="billformrow"><div class="cobhl cobhl2 cdformtaddress<%=IIfVs(isshp="" AND errordaddress," ectwarning")%>"><%=redstar & labeltxt(xxAddress,isshp&"address")%>:</div><div class="cobll cobll2 cdformaddress"><input type="text" name="<%=isshp%>address" id="<%=isshp%>address" class="cdform1fixw" placeholder="<%=stripnspecials(xxAddress)%>" /></div></div>
			<%	if useaddressline2=TRUE then %>
			<div class="billformrow"><div class="cobhl cobhl2 cdformtaddress2"><%=labeltxt(xxAddress2,isshp&"address2")%>:</div><div class="cobll cobll2 cdformaddress2"><input type="text" name="<%=isshp%>address2" id="<%=isshp%>address2" class="cdform1fixw" placeholder="<%=stripnspecials(xxAddress2)%>" /></div></div>
			<%	end if %>
			<div class="billformrow"><div class="cobhl cobhl2 cdformtcity<%=IIfVs(isshp="" AND errordcity," ectwarning")%>"><%=redstar & labeltxt(xxCity,isshp&"city")%>:</div><div class="cobll cobll2 cdformcity"><input type="text" name="<%=isshp%>city" id="<%=isshp%>city" class="cdform1fixw" placeholder="<%=stripnspecials(xxCity)%>" /></div></div>
			<div class="billformrow"><div class="cobhl cobhl2 cdformtstate<%=IIfVs((isshp="s" AND errordshipstate) OR (isshp<>"s" AND errordstate)," ectwarning")%>"><%=replace(redstar,"<span","<span id="""&isshp&"statestar""")%><label class="ectlabel" for="<%=isshp%>state" id="<%=isshp%>statetxt"><%=xxState%></label>:</div><div class="cobll cobll2 cdformstate"><% if hasstates then %><select name="<%=isshp%>state" id="<%=isshp%>state" size="1" onchange="dosavestate('')" class="cdform1fixw"><% havestate=show_states(-1) %></select><% end if %><input type="text" name="<%=isshp%>state2" id="<%=isshp%>state2" class="cdform1fixw" /></div></div>
			<div class="nohidebillrow"><div class="cobhl cobhl2 cdformtcountry<%=IIfVs(isshp="" AND errordcountry," ectwarning")%>"><%=redstar & labeltxt(xxCountry,isshp&"country")%>:</div><div class="cobll cobll2 cdformcountry"><select name="<%=isshp%>country" id="<%=isshp%>country" size="1" onchange="checkoutspan('<%=isshp%>');showshipform(1,this)" class="cdform1fixw"><% call show_countries(-1,TRUE) %></select></div></div>
			<div class="billformrow"><div class="cobhl cobhl2 cdformtzip<%=IIfVs(isshp="" AND errordzip," ectwarning")%>"><%=replace(redstar,"<span","<span id="""&isshp&"zipstar""") & "<label class=""ectlabel"" for="""&isshp&"zip"" id="""&isshp&"ziptxt"">" & xxZip & "</label>"%>:</div><div class="cobll cobll2 cdformzip"><input type="text" name="<%=isshp%>zip" id="<%=isshp%>zip" class="cdform1fixw" placeholder="<%=stripnspecials(xxZip)%>" autocapitalize="characters" /></div></div>
			<div class="billformrow"><div class="cobhl cobhl2 cdformtphone<%=IIfVs(isshp="" AND errordphone," ectwarning")%>"><% if isshp="" then print redstar %><%=labeltxt(xxPhone,isshp&"phone")%>:</div><div class="cobll cobll2 cdformphone"><input type="tel" name="<%=isshp%>phone" id="<%=isshp%>phone" class="cdform1fixw" placeholder="<%=stripnspecials(xxPhone)%>" /></div></div>
			<%	if trim(extraorderfield2)<>"" then %>
			<div class="billformrow"><div class="cobhl cobhl2 cdformtextra2"><%=IIfVr(extraorderfield2required=true,redstar,"") & labeltxt(extraorderfield2,"ord"&isshp&"extra2")%>:</div><div class="cobll cobll2 cdformextra2"><% if extraorderfield2html<>"" then print replace(replace(extraorderfield2html,"ectfield","ord"&isshp&"extra2"),"ordextra2","ord"&isshp&"extra2") else print "<input type=""text"" name=""ord"&isshp&"extra2"" id=""ord"&isshp&"extra2"" class=""cdform1fixw"" placeholder="""&stripnspecials(extraorderfield2)&""" autocomplete=""false"" />"%></div></div>
			<%	end if %>
...
<%		end if
		...
		function displayzip(isship) %>
				<div class="<%=IIfVr(isship="s","ship","bill")%>formrow">
				  ...
				  <div class="cobll cobll2 cdformzip"><input type="text" name="<%=isship%>zip" class="cdform2fixw cdforminput cdformzip" id="<%=isship%>zip" value="<%=htmlspecials(IIfVr(isship="s",ordShipZip,ordZip))%>" placeholder="<%=stripnspecials(xxZip)%>" autocapitalize="characters" /></div>
				</div>
<%		end function
		if trim(extraorderfield1)<>"" then %>
				<div class="billformrow" style="display:none">
				  ...
				  <div class="cobll cobll2 cdformextra1"><% if extraorderfield1html<>"" then print replace(extraorderfield1html,"ectfield","ordextra1") else print "<input type=""text"" name=""ordextra1"" class=""cdform2fixw cdforminput cdformextra1"" id=""ordextra1"" placeholder="""&stripnspecials(extraorderfield1)&""" value="""&htmlspecials(ordExtra1)&""" autocomplete=""false"" />"%></div>
				</div>
<%		end if %>
				<div class="billformrow" style="display:none">
		...
		if usefirstlastname then
			print "<input type=""text"" name=""ordname"" autocomplete=""given-name"" class=""cdformsmfixw cdforminput cdformname"" id=""ordname"" value="""&htmlspecials(ordName)&""" placeholder="""&stripnspecials(xxFirNam)&""" /> <input type=""text"" name=""lastname"" id=""lastname"" autocomplete=""family-name"" class=""cdformsmfixw cdforminput cdformlastname"" value="""&htmlspecials(ordLastName)&""" placeholder="""&stripnspecials(xxLasNam)&""" />"
		else
			print "<input type=""text"" name=""ordname"" class=""cdform2fixw cdforminput cdformname"" id=""ordname"" placeholder=""" & stripnspecials(xxName) & """ value="""&htmlspecials(ordName)&""" />"
		end if %></div>
				</div>
				<div class="billformrow" style="display:none">
				  <div class="cobhl cobhl2 cdformtemail<%=IIfVs(errordemail," ectwarning")%>"><%=redstar & labeltxt(xxEmail,"email")%></div>
				  <div class="cobll cobll2 cdformemail"><input type="email" name="email" class="cdform2fixw cdforminput cdformemail" id="email" placeholder="<%=stripnspecials(xxEmail)%>" value="<%=htmlspecials(ordEmail)%>" /></div>
				</div>
<%		if verifyemail then %>
				<div class="billformrow" style="display:none">
				  <div class="cobhl cobhl2 cdformtemail<%=IIfVs(errordemailv," ectwarning")%>"><%=redstar & labeltxt(xxEmVerf,"email2")%></div>
				  <div class="cobll cobll2 cdformemail"><input type="email" name="email2" class="cdform2fixw cdforminput cdformemail" id="email2" placeholder="<%=stripnspecials(xxEmVerf)%>" value="<%=htmlspecials(ordEmail2)%>" /></div>
				</div>
<%		end if %>
				<div class="billformrow" style="display:none">
				  <div class="cobhl cobhl2 cdformtaddress<%=IIfVs(errordaddress," ectwarning")%>"><%=redstar & labeltxt(xxAddress,"address")%></div>
				  <div class="cobll cobll2 cdformaddress"><input type="text" name="address" class="cdform2fixw cdforminput cdformaddress" id="address" placeholder="<%=stripnspecials(xxAddress)%>" value="<%=htmlspecials(ordAddress)%>" /></div>
				</div>
<%		if useaddressline2=TRUE then %>
				<div class="billformrow" style="display:none">
				  <div class="cobhl cobhl2 cdformtaddress2"><%=labeltxt(xxAddress2,"address2")%></div>
				  <div class="cobll cobll2 cdformaddress2"><input type="text" name="address2" class="cdform2fixw cdforminput cdformaddress2" id="address2" placeholder="<%=stripnspecials(xxAddress2)%>" value="<%=htmlspecials(ordAddress2)%>" /></div>
				</div>
<%		end if
		if zipposition=4 then call displayzip("") %>
				<div class="billformrow" style="display:none">
				  ...
				  <div class="cobll cobll2 cdformcity"><input type="text" name="city" class="cdform2fixw cdforminput cdformcity" id="city" placeholder="<%=stripnspecials(xxCity)%>" value="<%=htmlspecials(ordCity)%>" /></div>
				</div>
<%		if zipposition=3 then call displayzip("") %>
				<div class="billformrow" style="display:none">
				  ...
				  <div class="cobll cobll2 cdformstate"><% if hasstates then %><select name="state" class="cdform2fixw cdformselect cdformstate" id="state" size="1" onchange="dosavestate('')"><% havestate=show_states(ordState) %></select><% end if %><input type="text" name="state2" class="cdform2fixw cdforminput cdformstate" id="state2" style="display:none" placeholder="<%=stripnspecials(xxState)%>" value="<% if not havestate then print htmlspecials(ordState)%>" /></div>
				</div>
<%		if zipposition=2 then call displayzip("") %>
				<div class="nohidebillrow">
				  ...
				  <div class="cobll cobll2 cdformcountry"><select name="country" class="cdform2fixw cdformselect cdformcountry" id="country" size="1" onchange="checkoutspan('');showshipform(1,this)" ><% call show_countries(ordCountry,TRUE) %></select></div>
				</div>
<%		if zipposition=1 then call displayzip("") %>
				<div class="billformrow" style="display:none">
				  ...
				  <div class="cobll cobll2 cdformphone"><input type="tel" name="phone" class="cdform2fixw cdforminput cdformphone" id="phone" placeholder="<%=stripnspecials(xxPhone)%>" value="<%=htmlspecials(ordPhone)%>" /></div>
				</div>
<%		if trim(extraorderfield2)<>"" then %>
				<div class="billformrow" style="display:none">
				  ...
				  <div class="cobll cobll2 cdformextra2"><% if extraorderfield2html<>"" then print replace(extraorderfield2html,"ectfield","ordextra2") else print "<input type=""text"" name=""ordextra2"" class=""cdform2fixw cdforminput cdformextra2"" id=""ordextra2"" placeholder="""&stripnspecials(extraorderfield2)&""" value="""&ordExtra2&""" autocomplete=""false"" />"%></div>
				</div>
<%		end if
		call writeshippingflags(2)
		if noshipaddress<>TRUE then %>
...
<%			end if %>
			...
			if usefirstlastname then
				print "<input type=""text"" name=""sordname"" class=""cdformsmfixw cdforminput cdformname"" id=""sordname"" value="""&htmlspecials(ordShipName)&""" placeholder="""&stripnspecials(xxFirNam)&""" /> <input type=""text"" name=""slastname"" class=""cdformsmfixw cdforminput cdformlastname"" id=""slastname"" value="""&htmlspecials(ordShipLastName)&""" placeholder="""&stripnspecials(xxLasNam)&""" />"
			else
				print "<input type=""text"" name=""sordname"" class=""cdform2fixw cdforminput cdformname"" id=""sordname"" value="""&htmlspecials(ordShipName)&""" />"
			end if %></div>
				</div>
				<div class="shipformrow" style="display:none">
				  ...
				  <div class="cobll cobll2 cdformaddress"><input type="text" name="saddress" class="cdform2fixw cdforminput cdformaddress" id="saddress" placeholder="<%=stripnspecials(xxAddress)%>" value="<%=htmlspecials(ordShipAddress)%>" /></div>
				</div>
<%			if useaddressline2=TRUE then %>
				<div class="shipformrow" style="display:none">
				  <div class="cobhl cobhl2 cdformtaddress2"><%=labeltxt(xxAddress2,"saddress2")%></div>
				  <div class="cobll cobll2 cdformaddress2"><input type="text" name="saddress2" class="cdform2fixw cdforminput cdformaddress2" id="saddress2" placeholder="<%=stripnspecials(xxAddress2)%>" value="<%=htmlspecials(ordShipAddress2)%>" /></div>
				</div>
<%			end if
			if zipposition=4 then call displayzip("s") %>
				<div class="shipformrow" style="display:none">
				  ...
				  <div class="cobll cobll2 cdformcity"><input type="text" name="scity" class="cdform2fixw cdforminput cdformcity" id="scity" placeholder="<%=stripnspecials(xxCity)%>" value="<%=htmlspecials(ordShipCity)%>" /></div>
				</div>
<%			if zipposition=2 then call displayzip("s") %>
				<div class="shipformrow" style="display:none">
				  ...
				  <div class="cobll cobll2 cdformcountry"><select name="scountry" class="cdform2fixw cdformselect cdformcountry" id="scountry" size="1" onchange="checkoutspan('s')"><option value=""><%=xxPlsSel%>...</option><% call show_countries(ordShipCountry,FALSE) %></select></div>
				</div>
<%			if zipposition=3 then call displayzip("s") %>
				<div class="shipformrow" style="display:none">
				  ...
				  <div class="cobll cobll2 cdformstate"><% if hasstates then %><select name="sstate" class="cdform2fixw cdformselect cdformstate" id="sstate" size="1" onchange="dosavestate('s')"><% havestate=show_states(ordShipState) %></select><% end if %><input type="text" name="sstate2" class="cdform2fixw cdforminput cdformstate" id="sstate2" style="display:none" placeholder="<%=stripnspecials(xxState)%>" value="<% if not havestate then print htmlspecials(ordShipState)%>" /></div>
				</div>
<%			if zipposition=1 then call displayzip("s") %>
				<div class="shipformrow" style="display:none">
				  <div class="cobhl cobhl2 cdformtphone"><%=labeltxt(xxPhone,"sphone")%></div>
				  <div class="cobll cobll2 cdformphone"><input type="tel" name="sphone" class="cdform2fixw cdforminput cdformphone" id="sphone" placeholder="<%=stripnspecials(xxPhone)%>" value="<%=htmlspecials(ordShipPhone)%>" /></div>
				</div>
<%			if trim(extraorderfield2)<>"" then %>
				<div class="shipformrow" style="display:none">
				  <div class="cobhl cobhl2 cdformtextra2"><% if extraorderfield2required=TRUE then print redstar
									print labeltxt(extraorderfield2,"ordsextra2")%></div>
				  <div class="cobll cobll2 cdformextra2"><% if extraorderfield2html<>"" then print replace(replace(extraorderfield2html,"ordextra2","ordsextra2"),"ectfield","ordsextra2") else print "<input type=""text"" name=""ordsextra2"" class=""cdform2fixw cdforminput cdformextra2"" id=""ordsextra2"" placeholder="""&stripnspecials(extraorderfield2)&""" value="""&ordShipExtra2&""" autocomplete=""false"" />"%></div>
				</div>
<%			end if
...
<%	end if
	if trim(extracheckoutfield1)<>"" then
		...
		checkoutfield2=IIfVr(extracheckoutfield1html<>"", replace(extracheckoutfield1html,"ectfield","ordcheckoutextra1"), "<input type=""text"" name=""ordcheckoutextra1"" class=""cdform2fixw cdforminput cdformextraco1"" id=""ordcheckoutextra1"" size=""20"" value="""&htmlspecials(ordCheckoutExtra1)&""" placeholder=""" & htmlspecials(strip_tags2(extracheckoutfield1)) & """autocomplete=""false"" />")
%>				<div class="checkoutadditionals checkoutadds2col">
...
<%	end if
	if trim(extracheckoutfield2)<>"" then
		...
		checkoutfield2=IIfVr(extracheckoutfield2html<>"", replace(extracheckoutfield2html,"ectfield","ordcheckoutextra2"), "<input type=""text"" name=""ordcheckoutextra2"" class=""cdform2fixw cdforminput cdformextraco2"" id=""ordcheckoutextra2"" size=""20"" value="""&htmlspecials(ordCheckoutExtra2)&""" placeholder=""" & htmlspecials(strip_tags2(extracheckoutfield2)) & """ autocomplete=""false"" />")
%>				<div class="checkoutadditionals checkoutadds2col">
...
<%		elseif mailinglistradios then %>
					<div><%=xxYes%>&nbsp;<input type="radio" id="allowemailradioy" name="allowemail" value="ON" /></div>
					<div><%=xxNo%>&nbsp;<input type="radio" id="allowemailradion" name="allowemail" value="" /></div>
<%		else %>
					<input type="checkbox" name="allowemail" class="cdformcb cdformmailing" value="ON" <% if allowemaildefaulton=TRUE then print "checked=""checked"""%> />
<%		end if %>
				  </div>
...
<%	end if
	if termsandconditions then %>
				<div class="billformrowflags">
				  <div class="cdaddtflag cdformtterms"><input id="ecttnccheckbox" type="checkbox" name="license" class="cdformcb cdformterms" value="1" /></div>
...
<%	end if
	if nogiftcertificate<>TRUE then %>
			...
			<div><input type="text" name="cpncode" class="cdform2fixw cdforminput cdformcoupon" id="cpncode" placeholder="<%=stripnspecials(xxGifNum)%>" size="<%=IIfVr(mobilebrowser,14,20)%>" autocomplete="off" /> <%=imageorbutton(imgapplycoupon,xxApply,"applycoupon applycoupon2","applycert()",TRUE)%></div>
		<div id="cpncodespan"><%
...
<%	elseif UBOUND(alldata,2)=0 then
		...
		if payproviderradios=1 then
			...
			for rowcounter=0 to UBOUND(alldata,2)
				...
				print "<input type=""radio"" name=""payprovider"" id=""pprovid" & alldata(0,rowcounter) & """ class=""cdformradio cdformpayment"" value=""" & alldata(0,rowcounter) & """"
			...
			if payproviderradios=2 then
				...
				for rowcounter=0 to UBOUND(alldata,2)
					print "<label class=""ectlable cdformradio""><input type=""radio"" name=""payprovider"" class=""cdformradio cdformpayment"" value="""&alldata(0,rowcounter)&""""
					if ordPayProvider=alldata(0,rowcounter) OR (ordPayProvider="" AND nodefaultpayprovider<>TRUE) then print " checked=""checked""" : ordPayProvider="-1"
				...
				print "<select name=""payprovider"" class=""cdform2fixw cdformselect cdformpayment"" size=""1"">"
...
if(frm.ordname.value==""||frm.ordname.value=="<%=jscheck(xxFirNam)%>") return(chkfocus(frm.ordname,"<%=jscheck(xxPlsEntr&" """&IIfVr(usefirstlastname, xxFirNam, xxName))%>\"."));
<%	if usefirstlastname=TRUE then %>
if(frm.lastname.value=="") return(chkfocus(frm.lastname,"<%=jscheck(xxPlsEntr&" """&xxLasNam)%>\"."));
<%	else %>
var regex=/ /;
if(!checkedfullname && !regex.test(frm.ordname.value)){
...
<%		if verifyemail then %>
if(!regex.test(frm.email2.value)){
	alert("<%=jscheck(xxEmVerf&"\n\n"&xxValEm)%>");
	frm.email2.focus();
	return(false);
}
if(frm.email.value!=frm.email2.value){
	alert("<%=jscheck(xxEmNoMa)%>");
	frm.email2.focus();
	return(false);
}
<%		end if
...
<%	if trim(extraorderfield1)<>"" AND extraorderfield1required=true then print "if(!chkextra(true,frm.ordsextra1,"""&jscheck(strip_tags2(extraorderfield1))&"""))return(false);" &vbCrLf %>
	if(!shipaddcan&&(frm.sordname.value==""||frm.sordname.value=="<%=xxFirNam%>")&&!chkconfship(frm.sordname,"<%=jscheck(xxPlsEntr&" """&xxShpDet&" / "&xxName)%>\"."+xxnoship)) return false;
...
<% end if
	...
	if mailinglistdropdown then %>
if(frm.allowemail.selectedIndex==0){
	alert("<%=jscheck(xxPlSePE)%>");
	frm.allowemail.focus();
	return(false);
}
<%	elseif mailinglistradios then %>
if(!(document.getElementById('allowemailradioy').checked||document.getElementById('allowemailradion').checked)){
	alert("<%=jscheck(xxPlSePE)%>");
	document.getElementById('allowemailradioy').focus();
	return(false);
}
<%	end if
	if termsandconditions then %>
			...
			if NOT amazonpaycheckout AND NOT nopriceanywhere then
				...
				if estimateshipping=TRUE then
					...
					if wantstateselector then
						...
						print "<select name=""state"" id=""state"" class=""ectselectinput nofixw cartstateselector" & IIfVs(invalidstate," ectwarning") & """ size=""1"" onchange=""document.getElementById('updateestimator').style.display=''"">" : show_states(shipstate) : print "</select>"
						print "</div></div>" & vbCrLf
					end if
					if wantcountryselector then
						...
						print "<select name=""country"" id=""country"" class=""ectselectinput nofixw"" size=""1"""&IIfVs(mobilebrowser," style=""font-size:10px""")&" onchange="""&IIfVs(wantstateselector OR wantzipselector,"dynamiccountries(this,'');")&"updateestimator('"&cartlistnumber&"')"">"
					...
					if wantzipselector then
						...
						print "<input type=""text"" name=""zip"" id=""zip"" class=""ecttextinput nofixw cartzipselector" & IIfVs(invalidzip," ectwarning") & """ size=""8"" value=""" & htmlspecials(destZip) & """ onkeydown=""document.getElementById('updateestimator').style.display=''"" autocapitalize=""characters"" />"
						print "</div></div>" & vbCrLf
					end if
