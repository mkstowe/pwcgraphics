...
<%					if showproductid=TRUE then print "<div class=""prodid"">" & sstrong & xxPrId & ": " & estrong & rs("pID") & "</div>"
					if xxManLab<>"" then
						if NOT IsNull(rs(getlangid("scName",131072))) then print "<div class=""prodmanufacturer"">" & sstrong & xxManLab & ": " & estrong & rs(getlangid("scName",131072)) & "</div>"
					end if
					if showproductsku<>"" AND trim(rs("pSKU")&"")<>"" then print "<div class=""prodsku"">" & sstrong & showproductsku & ": " & estrong & rs("pSKU") & "</div>"
					print sstrong & "<div class=""prodname"">"&startlink&rs(getlangid("pName",1))&endlink&xxDot
					if alldiscounts<>"" then print " <span class=""discountsapply"">"&xxDsApp&"</span></div>" & estrong & "<div class=""proddiscounts"">" & alldiscounts & "</div>" else print "</div>" & estrong
					if noapplydiscounts<>"" then print " <span class=""discountsnotapply"">"&xxDsNoAp&"</span></div>" & estrong & "<div class=""prodnoapplydiscounts"">" & noapplydiscounts & "</div>" else print "</div>" & estrong
...
<%		if noproductseparator<>TRUE then
			print "<tr><td colspan=""3"" class=""prodseparator"">" & IIfVr(prodseparator<>"", prodseparator, "<hr class=""prodseparator"" width=""70%"" align=""center""/>") & "</td></tr>"
		end if
		Count=Count+1
